...
test_that("P14.2: the set stops at the number of harmonics selected", {
  e <- cos_env()
  expect_equal(labels_of(e$fck_cosinor_model_set(1)),
               c("none + H1", "linear + H1", "log + H1", "exp_sat + H1"))
  # the user's own example: saturating exponential with 2 harmonics
  expect_equal(labels_of(e$fck_cosinor_model_set(2)),
               c("none + H1", "none + H1-H2",
                 "linear + H1", "linear + H1-H2",
                 "log + H1", "log + H1-H2",
                 "exp_sat + H1", "exp_sat + H1-H2"))
  expect_equal(length(e$fck_cosinor_model_set(3)), 12L)
  # nothing beyond the selection, ever
  for (k in 1:3) {
    ks <- vapply(e$fck_cosinor_model_set(k), function(m) m$k, integer(1))
    expect_lte(max(ks), k)
...
test_that("P14.2: no harmonic is ever offered without the ones below it", {
  e <- cos_env()
  for (k in 1:4) for (m in e$fck_cosinor_model_set(k)) {
...
test_that("P14.1: every trend the UI offers is in the candidate set", {
  ...
  for (tt in e$FCK_TREND_TYPES)
    expect_true(grepl(paste0('"', tt, '"'), ui, fixed = TRUE), info = tt)
  expect_true("log" %in% e$FCK_TREND_TYPES)      # the one that was missing
  trends <- unique(vapply(e$fck_cosinor_model_set(2), function(m) m$trend, character(1)))
  expect_setequal(trends, e$FCK_TREND_TYPES)
})
...
test_that("P14.1: the hardcoded grid is gone from the diagnostic", {
  ...
  expect_true(grepl("fck_cosinor_model_set(n_harmonics)", src, fixed = TRUE))
  ...
  expect_true(grepl("fck_model_npar(", src, fixed = TRUE))
})

test_that("P14: parameter counts are right for every candidate", {
  e <- cos_env()
  # MESOR + trend + 2 per harmonic
  expect_equal(e$fck_model_npar("none", 1), 3L)
  expect_equal(e$fck_model_npar("linear", 1), 4L)
  expect_equal(e$fck_model_npar("log", 1), 4L)      # was silently counted as 0
  expect_equal(e$fck_model_npar("exp_sat", 1), 5L)
  expect_equal(e$fck_model_npar("exp_sat", 2), 7L)
  expect_equal(e$fck_trend_npar("log"), 1L)
  # a nested pair must differ by exactly the 2 parameters of one harmonic
  for (tt in e$FCK_TREND_TYPES)
    expect_equal(e$fck_model_npar(tt, 2) - e$fck_model_npar(tt, 1), 2L)
})
...
test_that("P14: the report carries the model set, the choice and its caveat", {
  ...
  t2 <- paste(e$fck_apa_report(vals, list(), "T"), collapse = "\n")

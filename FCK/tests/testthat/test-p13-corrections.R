...
test_that("P13.2: the FoSR caveat describes the estimator that was used", {
  ...
  gam_vals <- base_vals(reg_model = list(
    ...
    r2_t = runif(20, .05, .3)))
  gtxt <- paste(e$fck_apa_report(gam_vals, list(), "T"), collapse = "\n")
  ...
  ols_vals <- base_vals(reg_model = list(
    ...
    r2_t = runif(20, .05, .3)))
  otxt <- paste(e$fck_apa_report(ols_vals, list(), "T"), collapse = "\n")
...
test_that("P13.3: missingness is reported with or without a smoothing step", {
  ...
  clean <- paste(e$fck_apa_report(list(data = d), list(), "T"), collapse = "\n")
  ...
  gappy <- paste(e$fck_apa_report(list(data = d2), list(), "T"), collapse = "\n")
  ...
  sm <- paste(e$fck_apa_report(list(data = d2, fill_status = fs), list(), "T"),
              collapse = "\n")
...
test_that("P13.4: the fANOVA states which curves it was computed on", {
  ...
  t1 <- paste(e$fck_apa_report(on_reg, list(), "T"), collapse = "\n")
  ...
  t2 <- paste(e$fck_apa_report(on_raw, list(), "T"), collapse = "\n")
  ...
  t3 <- paste(e$fck_apa_report(base_vals(fanova_results = fa), list(), "T"), collapse = "\n")
...
test_that("P13.4: landmark registration states how its landmarks were obtained", {
  ...
  atxt <- paste(e$fck_apa_report(auto, list(), "T"), collapse = "\n")
  ...
  man <- base_vals(warping_results = list(
    ...
    time_points = seq(0, 1, length.out = 50)))
  mtxt <- paste(e$fck_apa_report(man, list(), "T"), collapse = "\n")

...
test_that("P15.2: the stale grid description is gone from the UI", {
  ...
  expect_true(grepl("fck_cosinor_model_set(k)",
...
test_that("P15.2: the help text is built from the same set that gets fitted", {
  ...
  expect_equal(length(e$fck_cosinor_model_set(1)), 4L)
  expect_equal(length(e$fck_cosinor_model_set(2)), 8L)
  expect_equal(length(e$fck_cosinor_model_set(3)), 12L)
  labs <- vapply(e$fck_cosinor_model_set(2), function(m) m$label, character(1))
...
test_that("P15.4: the fitted model's own AIC/AICc/BIC are reported", {
  ...
  expect_true(grepl("fck_model_npar(trend_type, nh) + 1L", src, fixed = TRUE))
...
test_that("P15.4: parameter counts used by the readout are the shared rule", {
  ...
  expect_equal(e$fck_model_npar("exp_sat", 1) + 1L, 6L)
  expect_equal(e$fck_model_npar("none", 1) + 1L, 4L)
})

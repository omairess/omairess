# Porting notes: how `WaPaa1_3.R` + `CIRCAREG.R` became `DANCE/`
...
**Hand-merged** (the shared pipeline — `DANCE/ui/10_import.R`,
...
`tools/port_fck.py` is a **one-time port script kept for provenance**. `DANCE/`
...
coordinates. `DANCE/server/03_helpers_clock.R` adds a third, explicitly named
...
(`dance_cumulative_hours()`, unwrapped across midnight: 20, 22, 2, 8 → 0, 2, 6,
...
There is now one rule, in `DANCE/server/04_helpers_fd.R`: no smoothing means an
...
call `dance_ensure_fd_obj()`, which builds that representation **and says so** in
...
(`dance_settings`) rather than by reading the widgets at export time, which would
...
gone. `DANCE/server/91_session.R` writes one `.rds` holding the whole `values`
...
Resolution now happens in one place (`DANCE/server/06_helpers_posthoc.R`), driven
...
`dance_rm_column()` prefers the row-aligned `values$covariates` and, when it must
...
`dance_rhythm_from_coefs()` keeps the real `trend_type` for indexing and simply
...
*The fit ring dropped the trend.* `dance_rhythm_from_coefs()` computed the
...
`dance_rhythm_from_coefs()` gains `include_trend` and `t_offset` and now mirrors
...
*The averaged-signal ring was rotated by the start hour.* `dance_profile_rings()`
took `dance_cumulative_hours()` — which returns **elapsed** hours from the first
column, starting at 0 — and passed it to `dance_wrap_to_clock()` as if it were a
...
It now reads the clock hour that `dance_clock_hours()` already parsed per column
and uses `cum` only for the day index.

`dance_open_path()` and `dance_band_path()` are the unsorted counterparts of
`dance_close_ring()` and `dance_band_ring()`. Sorting is right for a periodic ring
...
* **The pooled fitted equation dropped the homeostatic term** for *every* trend
  ...
  only the pooled line was wrong. One renderer now, `dance_format_equation()`.
...
is replaced by `dance_group_linear_test()`, which handles any number of groups.
...
transform: `dance_clock_label()` and `dance_clock_ticks()` in
...
routes through `dance_open_path`/`dance_band_path` rather than
`dance_close_ring`/`dance_band_ring`, and a test asserts the clock hours are
...
`dance_bounds_hit()` reports bounds **per parameter and per side**, on a relative
tolerance so a ceiling of 110 and a floor of 0.5 are judged alike;
`dance_bounds_summary()` rolls the per-subject lists into the two tables the
...
`dance_model_to_clock()`. Applied to the fit plot's tick text (positions stay on
...
One helper does the conversion now, `dance_acrophase_clock()` /
`dance_acrophase_label()`, and no site does its own arithmetic. The reporter's
...
acrophase can look wrong against its own figure. `dance_curve_peak_clock()`
...
and `DANCE_DENSITY_COLORS` in the polar density — so the same age band was a
...
`dance_group_colors()` keys on the level **name** through a stable sorted
...
"do the groups differ in timing". `dance_warp_params()` extracts the per-curve
...
`dance_group_dashes()` took its reference order from the *input* order rather than
the sorted order the colours use, so a group's dash changed when levels arrived
differently even though its colour did not; and `dance_warp_params()` assigned
...
`DANCE_PALETTE_VARIANT` is `"supplied"` — the eight hexes verbatim, because that
...
`DANCE_EMPHASIS` (dark neutral, the summary line), `DANCE_NEUTRAL` (identity lines,
the confidence ellipse), `DANCE_SERIES1`/`DANCE_SERIES2` (the ungrouped series) and
`DANCE_ALERT` (status *text*, not a series) are deliberately outside the
categorical ramp. The harmonic-component cycle
`c("green","orange","purple","brown",…)` — unvalidated CSS names — became
`dance_component_colors()`, drawn from the same ramp so one screen never mixes two
colour vocabularies.
...
`dance_auto_lambda()` (server/04_helpers_fd.R) now runs a real GCV search with the
...
**14.6% rejection at nominal 5%, 4.5% at nominal 1%**. `dance_reduced_exp_sat_sse()`
...
subject won every model comparison it entered. `dance_r_squared()` returns NA when
SST = 0 (undefined, not 1 and not 0) and `dance_gaussian_loglik()` floors sigma^2
...
points gave 4.95 and 9.97. `dance_l2_norm()` uses trapezoidal weights, so the same
...
than absorbed. `DANCE_RMFANOVA_DEFAULT` shows Cn_P1, Dn_P1 and En_P1; the other
...
every candidate basis size, using `dance_auto_lambda` &mdash; the same search the
...
now lives in `server/07_helpers_fosr.R` as `dance_fit_fosr_ols()`, is called by
...
`perform_functional_anova` &rarr; `dance_l2_norm`; `perform_rm_fanova` &rarr;
`dance_l2_norm`, `dance_rmfanova_global`, `dance_rm_design`; `fit_cosinor` &rarr;
...
`dance_auto_lambda` written out so the search can be re-run on new data.
...
`dance_fosr_design()` is the single place that turns new data into a design
...
`dance_fit_fosr_gam()`, emitted verbatim like the others, with the spline
...
the pattern `dance_auto_lambda()` already used.
...
`1:min(3, length(varprop))` on top of that. Now `dance_n_harmonics()`, which reads
...
included. **And this was not only a display number**: `dance_warp_params()` fed
...
`dance_warp_params()`. There was a third body, in the table, and it was missed.
...
`dance_warp_amplitude()` in `server/04_helpers_fd.R`, called by the table, the
...
over it: `dance_smoothing_axis()` and `dance_smoothing_basis()` now serve both, and
...
has been the smoothing configuration. P9.3 extracted `dance_smoothing_basis()` for
...
described. `dance_smoothing_nbasis()` in `server/04_helpers_fd.R` is now the single
...
`dance_landmark_warp()` was defined at **depth 11**, inside the
...
```
is dance_landmark_warp reachable from landmark_alignment_simple?  FALSE
Error in landmark_alignment: could not find function "dance_landmark_warp"
...
`dance_smoothing_axis()` and uses elapsed hours when real-time smoothing is on.
...
that is now one call as well: `dance_smoothing_axis()` is the only derivation, and
...
`dance_auto_lambda()` capped its objective at 60 subjects, taken as a systematic
...
*(a) The linear fitter never computed the Bingham region.* `dance_bingham_ci()`
...
*(c) `dance_apa_M()` had no `bounded` argument*, so *R*-squared printed as
...
fANOVA post-hoc object, assigned near the top of `dance_apa_report()` and read much
...
`dance_cosinor_model_set()`, `DANCE_TREND_TYPES`, `dance_trend_npar()`,
`dance_model_npar()` and the label helpers live in `server/08_helpers_cosinor.R`,
...
`dance_cosinor_model_set()` -- the same function that builds the set that gets
...
coming from the shared `dance_model_npar()`, and flags that tau was estimated
...
*The analyses.* P12.1 removed the 60-subject cap from `dance_auto_lambda()`, which
...
* ~~The CV basis-count divergence in 4.5.~~ **Closed at 4.57**: both
  paths now take their basis count from `dance_smoothing_nbasis()`.

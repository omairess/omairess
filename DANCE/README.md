...
```r
Rscript tests/smoke_test.R           # from the DANCE directory
...
Rscript tests/traj_tab6_test.R            # the Group / Condition Comparison pipeline
...
Every plot sits in a box with a drag handle in its bottom-right corner. The
default heights are starting points, not decisions: drag any plot to the size
the figure actually needs. Plotly charts redraw immediately; base-R plots are
regenerated by Shiny at the new dimensions, so text and axes stay the right size
rather than being stretched.

## Mixed designs (between x within)

The Functional ANOVA tab handles one factor: either between subjects or repeated
within them. If your design has **both** — a group that varies between
participants and a condition that varies within them — use the **Mixed design**
tab. It needs a participant identifier, which is what makes the design mixed.

Do **not** reach for the between-subjects test instead. A participant who
contributes two conditions appears as two rows, the permutation treats them as
independent curves, and the result is anticonservative. The app warns you on
import when identifiers repeat; this tab is what that warning points at.
...
```r
Rscript tools/renv_bootstrap.R   # from the DANCE directory; writes renv.lock
```

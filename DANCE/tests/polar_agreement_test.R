...
#   2. The signal profile wrapped ELAPSED hours (dance_cumulative_hours(), which
...
#
# Run with:   Rscript tests/polar_agreement_test.R      (from the DANCE directory)
# Needs no packages.
# ==============================================================================
...
# dance_clock_hours() calls the ported WaPaa parser; the merged app supplies it.
...
# ==============================================================================
# 1. dance_rhythm_from_coefs() vs predict_from_coefs()
# ==============================================================================
...
check("no trend: polar equals the fit plot",
      near(dance_rhythm_from_coefs(cf0, t_abs, 24, 1, "none", include_trend = TRUE),
           predict_from_coefs_ref(cf0, t_abs, 24, 1, "none")))
check("no trend: include_trend makes no difference",
      near(dance_rhythm_from_coefs(cf0, t_abs, 24, 1, "none", include_trend = TRUE),
           dance_rhythm_from_coefs(cf0, t_abs, 24, 1, "none", include_trend = FALSE)))
...
check("linear trend: include_trend = TRUE reproduces the fit plot",
      near(dance_rhythm_from_coefs(cf1, t_abs, 24, 1, "linear", include_trend = TRUE), ref))

no_trend <- dance_rhythm_from_coefs(cf1, t_abs, 24, 1, "linear", include_trend = FALSE)
...
check("exp_sat trend, 2 harmonics: matches the fit plot",
      near(dance_rhythm_from_coefs(cf2, t_abs, 24, 2, "exp_sat",
...
check("the trend-free curve closes: value at 00:00 equals value at 24:00",
      near(dance_rhythm_from_coefs(cf1, 0, 24, 1, "linear"),
           dance_rhythm_from_coefs(cf1, 24, 24, 1, "linear")))
check("the fit does NOT close, which is why it is drawn as an open arc",
      !near(dance_rhythm_from_coefs(cf1, 8, 24, 1, "linear", include_trend = TRUE),
            dance_rhythm_from_coefs(cf1, 32, 24, 1, "linear", include_trend = TRUE)))
...
op <- dance_open_path(arc_h, arc_v)
check("dance_open_path keeps the order it was given", identical(op$hours, arc_h))
check("dance_open_path does not repeat a point to close a seam",
      length(op$hours) == length(arc_h))
cr <- dance_close_ring(arc_h, arc_v)
check("dance_close_ring, by contrast, does sort and close",
      cr$hours[1] == 0 && cr$hours[length(cr$hours)] == 0)

bp <- dance_band_path(arc_h, arc_v - 5, arc_v + 5)
check("dance_band_path runs out along the upper edge in order",
      identical(bp$hours[seq_along(arc_h)], arc_h))
check("dance_band_path returns along the lower edge reversed",
...
clock  <- dance_clock_hours(labels)
cum    <- dance_cumulative_hours(labels)
...
src <- paste(readLines(file.path(app_dir, "server/74_polar_density.R"), warn = FALSE),
             collapse = "\n")
prof <- sub(".*dance_profile_rings <- function", "", src)
prof <- sub("output\\$harmonic_density_plot.*", "", prof)
check("dance_profile_rings takes its clock from dance_clock_hours()",
      grepl("clock <- dance_clock_hours", prof, fixed = TRUE))
check("dance_profile_rings no longer wraps cum straight onto the dial",
      !grepl("w <- dance_wrap_to_clock(cum", prof, fixed = TRUE))

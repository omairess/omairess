...
#
# Run with:   Rscript tests/clock_helpers_test.R      (from the DANCE directory)
...
chk <- function(label, labs, expect_hours, expect_uneven, expect_cum = NULL) {
  h <- dance_clock_hours(labs)
  u <- dance_spacing_is_uneven(labs)
  cum <- dance_cumulative_hours(labs)
  bad <- character(0)
...
if (!is.null(dance_cumulative_hours(c("Base9h", "Base9h", "Base10h")))) {
  failures <- failures + 1

...
test_that("the L2 statistic integrates instead of summing over grid points", {
  ...
  a <- dance_l2_norm(v50,  seq(0, 1, length.out = 50))
  b <- dance_l2_norm(v200, seq(0, 1, length.out = 200))
  ...
  expect_true(is.finite(dance_l2_norm(rep(1, length(uneven)), uneven)))
  expect_equal(dance_l2_norm(rep(1, length(uneven)), uneven), 1, tolerance = 1e-9)

  expect_true(is.na(dance_l2_norm(rep(NA_real_, 5))))
  expect_equal(dance_l2_norm(c(3), 0), 3)
})
...
test_that("no fANOVA site still uses the grid-dependent vector norm", {
  ...
  expect_gte(length(gregexpr("dance_l2_norm(", src, fixed = TRUE)[[1]]), 6)
})
...
test_that("FoSR fits by QR with a rank check and no longer inverts the normal equations", {
  ...
  expect_true(grepl("dance_fit_fosr_ols(", code_of("server/70_fosr.R"), fixed = TRUE))
})
...
test_that("P3.2: the FoSR p-values come from the analytical SE and ignore the seed", {
  ...
  set.seed(1); a <- dance_fit_fosr_ols(Y, df, c("x", "g"), use_bootstrap = TRUE, n_boot = 60)
  set.seed(9); b <- dance_fit_fosr_ols(Y, df, c("x", "g"), use_bootstrap = TRUE, n_boot = 60)
...
test_that("the diagnostics no longer hand an mgcv smoothing parameter to fda", {
  ...
  expect_true(grepl("dance_auto_lambda(dat, axis$t_full, basis", src, fixed = TRUE))
  expect_false(grepl("dance_auto_lambda(dat, seq_len(n_time), basis", src, fixed = TRUE))
})

...
test_that("the fANOVA export deparses the app's own estimator", {
  ...
  expect_true(grepl('emit_kernel("dance_fit_fosr_ols")', src, fixed = TRUE))
...
test_that("the exported script records and checks the environment it ran in", {
  src <- code_of("server/90_export.R")
  expect_true(grepl("dance_recorded_versions", src, fixed = TRUE))
  expect_true(grepl("dance_check_env", src, fixed = TRUE))
...
test_that("every statistical kernel has a recorded contract", {
  needed <- c("fit_cosinor", "fit_cosinor_nonlinear", "dance_auto_lambda",
              "perform_functional_anova", "perform_rm_fanova",
              "linear_shift_alignment", "fit_fosr",
              "run_functional_clustering")
  expect_true(all(needed %in% names(DANCE_CONTRACTS)))
  ...
  for (k in names(DANCE_CONTRACTS))
    expect_true("orientation" %in% names(DANCE_CONTRACTS[[k]]) ||
                "estimand" %in% names(DANCE_CONTRACTS[[k]]), info = k)
  expect_error(dance_contract("no_such_kernel"), "No contract recorded")
})

test_that("the contracts record the facts the audit established", {
  expect_true(grepl("14.6%", DANCE_CONTRACTS$fit_cosinor_nonlinear$zero_amp))
  expect_true(grepl("22.8%", DANCE_CONTRACTS$perform_rm_fanova$estimator))
  expect_true(grepl("does NOT call rmfanova", DANCE_CONTRACTS$perform_rm_fanova$package))
  expect_true(grepl("p_value_L2 is NA", DANCE_CONTRACTS$perform_rm_fanova$global))
  expect_true(grepl("does not search", DANCE_CONTRACTS$dance_auto_lambda$warning))
  expect_true(grepl("Not simultaneous", DANCE_CONTRACTS$perform_functional_anova$intervals, fixed = TRUE))
  expect_null(DANCE_CONTRACTS$fit_sofr)   # the module was removed
})
...
test_that("the RM design is built as rmfanova documents it", {
  ...
  d <- dance_rm_design(curves, sid, rmf)
  ...
  d2 <- dance_rm_design(curves[, -drop], sid[-drop], rmf[-drop])
  ...
  d3 <- dance_rm_design(curves[, 1:4], sid[1:4], rmf[1:4])
  expect_null(d3$x)
...
test_that("only the calibrated subset is shown by default", {
  source(file.path(app_dir, "server/09b_helpers_rmfanova.R"), local = TRUE)
  expect_equal(DANCE_RMFANOVA_DEFAULT, c("Cn_P1", "Dn_P1", "En_P1"))
  # the two that rejected true nulls at ~3x nominal must never be in the default
  expect_length(intersect(DANCE_RMFANOVA_DEFAULT, DANCE_RMFANOVA_INFLATED), 0)
  expect_length(intersect(DANCE_RMFANOVA_DEFAULT, DANCE_RMFANOVA_NOPOWER), 0)
  expect_setequal(DANCE_RMFANOVA_INFLATED, c("En_P2", "En_B2"))
  expect_setequal(DANCE_RMFANOVA_NOPOWER, c("Cn_P2", "Cn_B2"))
})
...
test_that("the global test is optional and reports why it did not run", {
  ...
  source(file.path(app_dir, "server/09b_helpers_rmfanova.R"), local = TRUE)
  r <- dance_rmfanova_global(matrix(1, 5, 2), factor(c(1, 2)), factor(c("a", "a")))
  expect_false(r$ok)

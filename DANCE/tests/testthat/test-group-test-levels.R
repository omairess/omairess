...
test_that("fewer than two surviving groups is reported, not crashed on", {
  ...
  # dance_group_linear_test is the effect-size path and must decline politely
  expect_null(dance_group_linear_test(rnorm(30), g))
  expect_null(dance_group_trend_test(rnorm(30), g))
})

# ------------------------------------------------------- clock-time display
test_that("linear model time renders as clock time", {
  expect_equal(dance_clock_label(8, 24), "08:00")
  expect_equal(dance_clock_label(12.5, 24), "12:30")
  expect_equal(dance_clock_label(23.99, 24), "23:59")
...
test_that("the day marker appears only when the axis actually spans two days", {
  within_day <- dance_clock_label(c(8, 12, 18), 24)
  expect_false(any(grepl("+1d", within_day, fixed = TRUE)))
  across <- dance_clock_label(c(8, 24, 30), 24)
  expect_true(any(grepl("+1d", across, fixed = TRUE)))
  # 08:00 on day 1 and on day 2 must be distinguishable
  two <- dance_clock_label(c(8, 32), 24)
  expect_false(two[1] == two[2])
})

test_that("minutes roll into hours instead of printing :60", {
  expect_equal(dance_clock_label(8.999, 24), "09:00")     # not "08:60"
  expect_equal(dance_clock_label(23.999, 24), "00:00")   # no series, no suffix
  expect_equal(dance_clock_label(c(8, 23.999), 24)[2], "00:00 (+1d)")
  expect_false(any(grepl(":60", dance_clock_label(seq(0, 48, by = 1/601), 24), fixed = TRUE)))
})
...
test_that("the old label formula produced the malformed strings this replaces", {
  ...
  expect_equal(dance_clock_label(12.5, 24), "12:30")      # what is printed now
  # and it had no zero padding
  expect_equal(paste0(8 %% 24, ":00"), "8:00")
  expect_equal(dance_clock_label(8, 24, with_minutes = FALSE), "08:00")
})

test_that("ticks land on clock-friendly positions across the recording", {
  tk <- dance_clock_ticks(c(8, 30), 24)
...
test_that("tick generation copes with short and degenerate ranges", {
  expect_null(dance_clock_ticks(c(5, 5), 24))
  expect_null(dance_clock_ticks(c(NA, 30), 24))
  short <- dance_clock_ticks(c(8, 11), 24)
  expect_false(is.null(short))
  expect_true(all(short$vals >= 8 & short$vals <= 11))
})

test_that("a non-24 period still labels sensibly", {
  tk <- dance_clock_ticks(c(0, 12), 12)
...
test_that("clock labelling never changes the underlying linear coordinate", {
  ...
  lab <- dance_clock_label(t_lin, 24)
  tk <- dance_clock_ticks(range(t_lin), 24)

...
test_that("beta_0, the MESOR and the start value are three different numbers", {
  ...
  b0 <- coefs[1]
  v0 <- dance_value_at(coefs, mod, min(mod$time_vec))
  ram <- dance_rhythm_adjusted_mean(b0, "exp_sat", c(coefs[2], coefs[3]),
...
test_that("with no trend and no harmonics all three coincide", {
  ...
  coefs <- c(42, 0, 0)
  expect_equal(dance_value_at(coefs, mod, 0), 42)
  expect_equal(dance_rhythm_adjusted_mean(42, "none", numeric(0), 0, 24, 0), 42)
})

test_that("the MESOR is above beta_0 for a rising trend, as it must be", {
  ram <- dance_rhythm_adjusted_mean(20.53, "exp_sat", c(60, 14), 0, 22, 0)
  expect_gt(ram, 20.53)

...
test_that("model time converts back to the SAME clock time under either origin", {
  ...
  for (o in c("midnight", "first_observation")) {
    mod <- mk_mod(o)
    expect_equal(dance_model_to_clock(mod$time_vec, mod), linear_times, info = o)
  }
})
...
test_that("the first observation is 08:00 on the clock whichever origin is set", {
  for (o in c("midnight", "first_observation")) {
    mod <- mk_mod(o)
    lab <- dance_clock_label(dance_model_to_clock(mod$time_vec, mod), 24)
...
test_that("the unconverted axis is what produced the reported symptom", {
  ...
  wrong <- dance_clock_label(fst$time_vec, 24)
  expect_equal(wrong[1], "00:00")            # exactly what the user saw
  # the fix
  right <- dance_clock_label(dance_model_to_clock(fst$time_vec, fst), 24)
  ...
  mid <- mk_mod("midnight")
  expect_equal(dance_clock_label(mid$time_vec, 24),
               dance_clock_label(dance_model_to_clock(mid$time_vec, mid), 24))
})

test_that("ticks land on the model axis but read in clock time", {
  fst <- mk_mod("first_observation")
  shift <- fst$origin_shift
  tk <- dance_clock_ticks(range(fst$time_vec) + shift, 24)
  ...
  expect_equal(tk$text[1], dance_clock_label(tk$vals[1], 24))
})

test_that("the clock range covers the real recording under either origin", {
  for (o in c("midnight", "first_observation")) {
    mod <- mk_mod(o)
    expect_equal(dance_model_clock_range(mod), range(linear_times), info = o)
  }
})
...
test_that("midnight boundaries are clock events, not model events", {
  ...
  expect_equal(dance_clock_label(naive + shift, 24), "08:00")
...
test_that("laps are counted in clock days, not model days", {
  ...
  expect_equal(dance_clock_label(split_right + shift, 24), "00:00", tolerance = 0)
  expect_equal(dance_clock_label(split_wrong + shift, 24), "08:00")
...
test_that("nothing in the conversion touches the fitted values", {
  ...
  S_clock <- 1 - exp(-(dance_model_to_clock(fst$time_vec, fst) -
                       min(dance_model_to_clock(fst$time_vec, fst))) / 18)
...
test_that("a model with no origin_shift field behaves as unshifted", {
  ...
  expect_equal(dance_model_to_clock(mod$time_vec, mod), linear_times)
  expect_equal(dance_model_clock_range(mod), range(linear_times))
})

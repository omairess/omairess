...
test_that("the n-basis GCV sweep no longer scores auto mode at lambda = 0", {
  ...
  expect_true(grepl("dance_auto_lambda(data_mat, axis$t_full, basis", src, fixed = TRUE))
  expect_false(grepl("dance_auto_lambda(data_mat, seq_len(n_time), basis", src, fixed = TRUE))
...
test_that("P3.1: with lambda re-selected per basis, GCV is flatter than at lambda = 0", {
  ...
  gcv_auto <- vapply(nb_seq, function(nb) {
    b <- create.bspline.basis(c(1, n_t), nbasis = nb)
    dance_auto_lambda(dat, seq_len(n_t), b, n_grid = 12)$gcv
  }, numeric(1))
...
test_that("the FoSR estimator is one named function, called by the GUI", {
  ...
  expect_true(grepl("dance_fit_fosr_ols <- function", helper, fixed = TRUE))
  expect_true(grepl("dance_fit_fosr_ols(", gui, fixed = TRUE))
...
test_that("the export emits each kernel's helper dependency closure", {
  ...
  expect_true(grepl("dance_fn_order <- function", src, fixed = TRUE))
...
test_that("P3.5b: the export no longer calls an unpenalised fit 'REML'", {
  ...
  expect_true(grepl("lambda chosen by GCV (dance_auto_lambda)", src, fixed = TRUE))
  expect_true(grepl('emit_kernel("dance_auto_lambda")', src, fixed = TRUE))
})

...
source(file.path(app_dir, "server/07_helpers_circular.R"))   # dance_rhythm_from_coefs
...
test_that("the reported H1 acrophases convert to the stated clock times", {
  expect_equal(dance_acrophase_label(hours = 19.18, harmonic = 1, clock_origin = ORIGIN), "03:11")
  expect_equal(dance_acrophase_label(hours = 19.89, harmonic = 1, clock_origin = ORIGIN), "03:53")
  expect_equal(dance_acrophase_label(hours = 18.93, harmonic = 1, clock_origin = ORIGIN), "02:56")
})

test_that("H2 has two equivalent maxima and both are reported", {
  lab <- dance_acrophase_label(hours = 7.81, harmonic = 2, clock_origin = ORIGIN)
  expect_true(grepl("15:49", lab, fixed = TRUE))
  expect_true(grepl("03:49", lab, fixed = TRUE))
  a <- dance_acrophase_clock(hours = 7.81, harmonic = 2, clock_origin = ORIGIN)
...
test_that("the numeric conversion is the stated formula", {
  for (x in c(19.18, 19.89, 18.93, 0, 23.9)) {
    expect_equal(dance_acrophase_clock(hours = x, harmonic = 1, clock_origin = ORIGIN)$hours,
                 (x + ORIGIN) %% 24)
  }
})
...
test_that("radians convert with the harmonic divisor", {
  ...
  for (h in 1:3) {
    for (phi in c(0.7, 2.04, 4.5)) {
      expect_equal(dance_acrophase_clock(phi_rad = phi, harmonic = h,
...
test_that("radians and hours agree for the reporter's H2 value", {
  ...
  expect_equal(dance_acrophase_label(phi_rad = phi, harmonic = 2, clock_origin = ORIGIN),
               dance_acrophase_label(hours = 7.81, harmonic = 2, clock_origin = ORIGIN))
})
...
test_that("harmonic h reports exactly h maxima, evenly spaced", {
  for (h in 1:4) {
    a <- dance_acrophase_clock(hours = 3, harmonic = h, clock_origin = ORIGIN)
...
test_that("every maximum really is a maximum of that harmonic", {
  ...
  a <- dance_acrophase_clock(hours = elapsed, harmonic = h, clock_origin = ORIGIN)
...
test_that("a zero origin is a no-op", {
  for (x in c(0, 5.5, 19.18, 23.99))
    expect_equal(dance_acrophase_clock(hours = x, harmonic = 1, clock_origin = 0)$hours, x)
  expect_equal(dance_acrophase_label(hours = 19.18, harmonic = 1, clock_origin = 0), "19:11")
})

test_that("wrapping past midnight is handled, not clamped", {
  expect_equal(dance_acrophase_clock(hours = 20, harmonic = 1, clock_origin = 8)$hours, 4)
  expect_equal(dance_acrophase_clock(hours = 16, harmonic = 1, clock_origin = 8)$hours, 0)
  expect_equal(dance_acrophase_label(hours = 16, harmonic = 1, clock_origin = 8), "00:00")
})

test_that("NA and empty input give NA, not an error or a wrong number", {
  expect_true(is.na(dance_acrophase_clock(hours = NA_real_, clock_origin = 8)$hours))
  expect_true(is.na(dance_acrophase_label(hours = NA_real_, clock_origin = 8)))
  expect_null(dance_acrophase_clock())
})

test_that("the helper is vectorised", {
  v <- c(19.18, 19.89, 18.93)
  expect_equal(dance_acrophase_clock(hours = v, harmonic = 1, clock_origin = ORIGIN)$hours,
               (v + ORIGIN) %% 24)
  expect_equal(dance_acrophase_label(hours = v, harmonic = 1, clock_origin = ORIGIN),
               c("03:11", "03:53", "02:56"))
})

test_that("dance_clock_origin reads the model, and defaults to zero", {
  expect_equal(dance_clock_origin(list(origin_shift = 8)), 8)
  expect_equal(dance_clock_origin(list()), 0)          # midnight origin
  expect_equal(dance_clock_origin(NULL), 0)            # no model yet
})
...
test_that("the conversion is invariant for every DISPERSION and DIFFERENCE", {
  ...
  expect_equal(dance_rayleigh(r1$r_unweighted, r1$n)$Z,
               dance_rayleigh(r0$r_unweighted, r0$n)$Z, tolerance = 1e-12)
...
test_that("the fitted curve is untouched by the reporting change", {
  ...
  y <- dance_rhythm_from_coefs(coefs, tv, 24, 2, "exp_sat", include_trend = TRUE, t_offset = 0)
  expect_true(all(is.finite(y)))
  # converting for display must not enter the prediction
  y2 <- dance_rhythm_from_coefs(coefs, tv, 24, 2, "exp_sat", include_trend = TRUE, t_offset = 0)
...
test_that("the curve peak and the H1 acrophase are reported as different things", {
  ...
  pk <- dance_curve_peak_clock(coefs, mod)
...
test_that("a peak at the window edge is flagged as a boundary value", {
  ...
  pk <- dance_curve_peak_clock(c(10, 2, 0.001, 0.001), mod)

...
test_that("a group keeps its colour when another is filtered out", {
  full <- dance_group_colors(AGE)
  sub  <- dance_group_colors(c("YOUTH", "ELDERLY"), all_levels = AGE)
...
test_that("a group keeps its colour when the levels arrive in another order", {
  a <- dance_group_colors(AGE)
  b <- dance_group_colors(rev(AGE))
...
test_that("the assignment is by name, so it survives a differently-sorted tab", {
  a <- dance_group_colors(c("ADULT", "YOUTH"))
  b <- dance_group_colors(c("YOUTH", "ADULT"))
...
test_that("the prefix is the four medium members, one per hue family", {
  expect_equal(length(DANCE_GROUP_COLORS), 4)
  expect_true(all(grepl("^#[0-9a-fA-F]{6}$", DANCE_GROUP_COLORS)))
  expect_equal(DANCE_GROUP_COLORS, c("#00b0be", "#f45f74", "#98c127", "#ffb255"))
  # not the order they were listed in: that would put the two blues adjacent
  expect_false(identical(dance_group_ramp(2), c("#8fd7d7", "#00b0be")))
})

test_that("slots 5-8 are the light partners of slots 1-4", {
  expect_equal(DANCE_GROUP_COLORS_EXTRA, c("#8fd7d7", "#ff8ca1", "#bdd373", "#ffcd8e"))
  ...
  expect_setequal(dance_group_ramp(8), supplied)
})
...
test_that("the cvd_safe variant re-steps only the green and the orange", {
  ...
  expect_equal(.DANCE_MEDIUMS$cvd_safe[1:2], .DANCE_MEDIUMS$supplied[1:2])
  expect_false(identical(.DANCE_MEDIUMS$cvd_safe[3], .DANCE_MEDIUMS$supplied[3]))
  expect_false(identical(.DANCE_MEDIUMS$cvd_safe[4], .DANCE_MEDIUMS$supplied[4]))
  expect_equal(.DANCE_MEDIUMS$cvd_safe, c("#00b0be", "#f45f74", "#9ec055", "#d38400"))
  # an unrecognised variant name falls back rather than erroring at load
  expect_true(DANCE_PALETTE_VARIANT %in% names(.DANCE_MEDIUMS))
})

test_that("dash is required past the validated depth, and only past it", {
  expect_false(dance_needs_dash(4))
  expect_true(dance_needs_dash(5))
  # eight distinct patterns, so slot k and its light partner at k+4 differ
  expect_equal(length(unique(DANCE_GROUP_DASH)), 6)
  d <- dance_group_dashes(LETTERS[1:8])
...
test_that("the ramp extends past the palette rather than erroring", {
  expect_equal(length(dance_group_ramp(8)), 8)
  expect_equal(dance_group_ramp(3), DANCE_GROUP_COLORS[1:3])
  expect_equal(length(dance_group_ramp(12)), 12)   # recycles, with dash carrying identity
})

test_that("components draw from the same vocabulary as groups", {
  expect_equal(dance_component_colors(4), dance_group_ramp(4))
  expect_equal(length(dance_component_colors(8)), 8)
})
...
test_that("emphasis and reference roles sit OUTSIDE the categorical palette", {
  ...
  full <- dance_group_ramp(8)
  expect_false(DANCE_EMPHASIS %in% full)
  expect_false(DANCE_NEUTRAL %in% full)
  expect_equal(DANCE_SERIES1, unname(DANCE_GROUP_COLORS[1]))
  expect_equal(DANCE_SERIES2, unname(DANCE_GROUP_COLORS[2]))
})

test_that("dashes are keyed the same way as the colours", {
  d1 <- dance_group_dashes(AGE)
  d2 <- dance_group_dashes(rev(AGE))
...
test_that("fills and rgba derive from the colour, so they cannot drift", {
  hex <- DANCE_GROUP_COLORS[1]
  expect_equal(dance_group_fill(hex, 0.25), paste0(hex, "40"))
  expect_equal(dance_group_fill(hex, 1), paste0(hex, "FF"))
  expect_equal(dance_group_fill(hex, 0), paste0(hex, "00"))
  expect_true(grepl("^rgba\\(", dance_group_rgba(hex, 0.4)))
  # the rgba channels really are this colour's
  m <- grDevices::col2rgb(hex)
  expect_true(grepl(sprintf("rgba\\(%d,%d,%d", m[1], m[2], m[3]),
                    dance_group_rgba(hex, 0.4)))
})

test_that("alpha is clamped rather than producing invalid hex", {
  hex <- DANCE_GROUP_COLORS[1]
  expect_equal(dance_group_fill(hex, 5), paste0(hex, "FF"))
  expect_equal(dance_group_fill(hex, -1), paste0(hex, "00"))
})
...
test_that("the harmonic tab no longer hard-codes its own hues", {
  ...
  expect_true(grepl("dance_group_colors(", src, fixed = TRUE))
})
...
test_that("the polar density draws from the shared palette", {
  src <- paste(readLines(file.path(app_dir, "server/74_polar_density.R"), warn = FALSE),
               collapse = "\n")
  expect_true(grepl("DANCE_DENSITY_COLORS <- c(DANCE_GROUP_COLORS", src, fixed = TRUE))
  # and keys on name, not position
  expect_false(grepl("DANCE_DENSITY_COLORS[((i - 1)", src, fixed = TRUE))
})
...
test_that("warping parameters are extracted for a shift-based method", {
  ...
  wp <- dance_warp_params(list(shifts = sh, warp_functions = wf, time_points = tp))
...
test_that("a warp that moved nothing yields nothing to test", {
  ...
  expect_null(dance_warp_params(list(shifts = rep(0, 4), warp_functions = wf, time_points = tp)))
  expect_null(dance_warp_params(NULL))
})
...
test_that("warp amplitude works for a nonlinear warp with no single shift", {
  ...
  wp <- dance_warp_params(list(warp_functions = wf, time_points = tp))
  expect_false(is.null(wp))
...
test_that("the warping parameters run through the same ANOVA machinery", {
  ...
  r <- dance_pc_anova_all(as.matrix(wp), g, across_pc_correction = "holm")
  expect_equal(r$k, 1)
...
test_that("no unvalidated CSS colour names are left carrying series identity", {
  ...
  expect_true(grepl("dance_component_colors(", h, fixed = TRUE))
})

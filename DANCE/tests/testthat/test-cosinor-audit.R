...
#
# Run:  Rscript -e 'testthat::test_dir("tests/testthat")'    (from DANCE/)
...
test_that("the Rayleigh Z comes from the UNWEIGHTED resultant", {
  ...
  expect_equal(dance_rayleigh(0.789, 1305)$Z, 812.3, tolerance = 0.2)
  ...
  expect_false(isTRUE(all.equal(dance_rayleigh(0.789, 1305)$Z, 1305 * 0.824^2, tolerance = 1e-3)))
})

test_that("dance_resultants separates the two resultants and never swaps them", {
  set.seed(42)
  n <- 400
  phi <- rnorm(n, mean = 0.70, sd = 0.69)
  amp <- rgamma(n, shape = 4, scale = 6)
  r <- dance_resultants(phi, amp)
  ...
  expect_equal(dance_rayleigh(r$r_unweighted, r$n)$Z, r$n * r$r_unweighted^2, tolerance = 1e-12)
})

test_that("with unit amplitudes the two resultants coincide", {
  phi <- c(0.1, 0.4, 0.9, 1.4)
  r <- dance_resultants(phi, rep(1, 4))
...
test_that("commonality partitions total R2 exactly, for every subject", {
  cm <- dance_commonality(0.892, 0.283, 0.838)
  ...
  for (i in 1:200) {
    ...
    cm <- dance_commonality(tot, s_only, c_only)
...
test_that("a negative shared component is reported, not clamped", {
  cm <- dance_commonality(0.90, 0.30, 0.40)   # suppression: total exceeds both marginals
...
test_that("the rhythm-adjusted mean integrates S(t) on the anchoring the fitter uses", {
  ...
  expect_equal(dance_rhythm_adjusted_mean(27.70, "exp_sat", c(32.302, 13.92), 8, 30, 8),
  ...
  expect_equal(dance_rhythm_adjusted_mean(27.70, "exp_sat", c(32.302, 13.92), 8, 30, 0),
               50.87, tolerance = 0.02)
  # with no trend the rhythm-adjusted mean IS the intercept
  expect_equal(dance_rhythm_adjusted_mean(27.70, "none", numeric(0), 8, 30, 8), 27.70)
})

test_that("the intercept is not the rhythm-adjusted mean when a trend is present", {
  ram <- dance_rhythm_adjusted_mean(27.70, "exp_sat", c(32.302, 13.92), 8, 30, 8)
...
test_that("the F test charges the harmonics only for what they add", {
  ...
  fixed <- dance_zero_amplitude_test(ss_full, ss_trend_only, 16, 8, 2)
...
test_that("the fitted equation always carries the homeostatic term", {
  eq <- dance_format_equation(27.70, "exp_sat", c(32.302, 13.92),
  ...
  expect_false(identical(
    eq, dance_format_equation(27.70, "none", NULL, c(23.84, 6.84), c(0.70, 2.04), 24, 0)))
})

test_that("the formatter renders the pooled and group equations identically", {
  a <- dance_format_equation(30.1, "exp_sat", c(28.0, 12.0), c(20, 5), c(0.6, 2.0), 24, 8)
  b <- dance_format_equation(30.1, "exp_sat", c(28.0, 12.0), c(20, 5), c(0.6, 2.0), 24, 8)
...
test_that("an unmatched group label is surfaced, not silently dropped", {
  ...
  ga <- dance_group_audit(labs, seq_along(labs))
...
test_that("groups too small to summarise are named rather than vanishing", {
  ...
  ga <- dance_group_audit(labs, seq_along(labs), min_n = 3)
...
test_that("the acrophase SE exists for a nonlinear fit", {
  se <- dance_acro_se(20, -12, matrix(c(4, 3, 3, 9), 2, 2))
...
test_that("Bingham refuses to quote a phase when the region covers the origin", {
  b <- dance_bingham_ci(0.2, 0.1, diag(c(100, 100)), n = 16, n_params = 8)
...
test_that("Bingham gives finite limits for a well-determined rhythm", {
  b <- dance_bingham_ci(20, -12, diag(c(1, 1)), n = 16, n_params = 8)
...
test_that("the linear group test returns effect sizes and an interval", {
  ...
  lt <- dance_group_linear_test(x, g)
  expect_equal(lt$k, 4)
...
test_that("Watson-Williams reports its concentration assumption", {
  expect_false(dance_ww_assumption(0.30)$ok)
  expect_true(dance_ww_assumption(0.80, kappa = 3)$ok)
  expect_true(grepl("0.45", dance_ww_assumption(0.30)$msg, fixed = TRUE))
})

# ------------------------------------------------------------- Akaike weights
test_that("the AICc table sorts by dAICc and its weights sum to 1", {
  tb <- dance_akaike_table(list(`none + H1` = 120, `exp_sat + H2` = 100, `linear + H2` = 104))
...
test_that("fitted values outside the admissible range are flagged", {
  bc <- dance_check_bounds(c(-3, 10, 40), lower = 0, upper = 100)
  expect_equal(bc$below, 1)
  expect_false(bc$ok)
  expect_true(dance_check_bounds(c(1, 10, 40), lower = 0, upper = 100)$ok)
})

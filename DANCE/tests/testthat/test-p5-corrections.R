...
test_that("P5.1: a fitted FoSR model can build a prediction design for new data", {
  ...
  fit <- dance_fit_fosr_ols(Y, df, c("Age (years)", "Sex"))
...
test_that("P5.1: prediction refuses new data it cannot honour, instead of blanking", {
  ...
  fit <- dance_fit_fosr_ols(Y, df, c("a", "g"))

  expect_error(dance_fosr_design(fit, data.frame(a = 1)), "missing predictor")
  expect_error(dance_fosr_design(fit, data.frame(a = 1, g = factor("z"))),
               "not seen when the model was fitted")
  # a single-level new frame still yields the full design (xlev is applied)
  ok <- dance_fosr_design(fit, data.frame(a = 1, g = factor("y", levels = c("x", "y"))))
...
test_that("P5.1: both prediction sites go through the shared builder", {
  ...
  expect_true(grepl("dance_fosr_design(mod, pred_df)", src, fixed = TRUE))
  expect_true(grepl("dance_fosr_design(mod, df_min)", src, fixed = TRUE))
  expect_true(grepl("dance_fosr_design(mod, df_max)", src, fixed = TRUE))
})
...
test_that("P5.5: a landmark warp is built and validated in one place", {
  src <- warp_code()
  expect_true(grepl("dance_landmark_warp <- function(ref, own, time_points)", src, fixed = TRUE))
  ...
  # "dance_landmark_warp <- function(", so it does not match this pattern)
  expect_equal(length(gregexpr("dance_landmark_warp(", src, fixed = TRUE)[[1]]), 2L)
})
...
test_that("P5.5: crossed or duplicated landmarks are rejected, not folded", {
  ...
  # P11.1: dance_landmark_warp() moved to the pure kernel file. It is a plain
  ...
  walk <- function(e) {
    ...
    if (length(e) == 3 && as.character(e[[1]]) %in% c("<-", "=") &&
        is.name(e[[2]]) && identical(as.character(e[[2]]), "dance_landmark_warp")) {
  ...
  expect_true(got)
  f <- get("dance_landmark_warp", envir = src_env)
...
test_that("P5.8: the GAM estimator is a kernel the export emits verbatim", {
  ...
  expect_true(grepl("dance_fit_fosr_gam <- function", helper, fixed = TRUE))
  ...
  expect_true(grepl("dance_fit_fosr_gam(", code_of("server/70_fosr.R"), fixed = TRUE))
  exp_src <- code_of("server/90_export.R")
  expect_true(grepl('emit_kernel("dance_fit_fosr_gam")', exp_src, fixed = TRUE))

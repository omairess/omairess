...
#          app: dance_landmark_warp() was defined at depth 11 inside the landmark
...
test_that("P11.2: every registration kernel is a top-level, reachable definition", {
  e <- kern_env()
  for (f in c("dance_landmark_warp", "linear_shift_alignment",
  ...
  # helper. While dance_landmark_warp lived in an observer's frame it did not.
  expect_identical(environment(e$landmark_alignment_simple),
                   environment(e$dance_landmark_warp))
})
...
test_that("P11.3: the diagnostics build no axis or production basis of their own", {
  ...
  expect_true(grepl("axis            <- dance_smoothing_axis(input, values)",
                    code_of("server/20_smoothing.R"), fixed = TRUE))
  dg <- code_of("server/30_diagnostics.R")
  expect_true(grepl("axis  <- dance_smoothing_axis(input, values)", dg, fixed = TRUE))
  expect_true(grepl("axis   <- dance_smoothing_axis(input, values)", dg, fixed = TRUE))
  # argvals must be the axis the basis was built over, in both sweep branches
  expect_true(grepl("dance_auto_lambda(data_mat, axis$t_full, basis", dg, fixed = TRUE))
...
test_that("P11.3: production and diagnostic bases agree on every configuration", {
  ...
  for (real in c(FALSE, TRUE)) for (cyc in c(FALSE, TRUE)) {
    ...
    b    <- e$dance_smoothing_basis(axis, e$dance_smoothing_nbasis(inp, n_time), "manual")
...
test_that("P11.3: nb_fourier varies only the count, and only for the sweep", {
  ...
  inp  <- list(use_real_time = TRUE, is_cyclic = TRUE, smooth_method = "manual",
               n_basis = 12, n_basis_manual = 12)
  axis <- e$dance_smoothing_axis(inp, vals)
  prod <- e$dance_smoothing_basis(axis, 12, "manual")
  expect_equal(e$dance_smoothing_basis(axis, 12, "manual", nb_fourier = NULL)$nbasis,
               prod$nbasis)

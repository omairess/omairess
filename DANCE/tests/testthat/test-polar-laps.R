...
# dance_clock_hours() calls the ported WaPaa column-name parser, which lives in
...
# Lift dance_fit_rings and dance_profile_rings out of the shipped file so the tests
...
# dance_profile_rings then silently returns NULL because dance_clock_hours cannot
...
dance_fit_rings     <- .load_fn("dance_fit_rings")
dance_profile_rings <- .load_fn("dance_profile_rings")
...
test_that("the polar fit equals the 2-D fit at every point, per group", {
  mod <- mk_mod(span = 26)
  fr <- dance_fit_rings(list(density_fit_scope = "recording", density_lap_mode = "spiral",
...
test_that("a recording longer than a period is flagged as lapping", {
  expect_true(dance_fit_rings(list(density_fit_scope = "recording"),
                            list(harmonic_model = mk_mod(span = 26)))$spans_period)
  expect_false(dance_fit_rings(list(density_fit_scope = "recording"),
...
test_that("per_lap splits into one trace per day and loses nothing", {
  ...
  sp <- dance_fit_rings(list(density_fit_scope = "recording", density_lap_mode = "spiral"),
                      list(harmonic_model = mod))
  pl <- dance_fit_rings(list(density_fit_scope = "recording", density_lap_mode = "per_lap"),
                      list(harmonic_model = mod))
...
test_that("folding onto one dial averages the repeated clock times", {
  v <- mk_vals()
  pr <- dance_profile_rings(list(density_profile_mode = "average"), v)
...
test_that("the spiral keeps every point and averages nothing across days", {
  v <- mk_vals()
  pr <- dance_profile_rings(list(density_profile_mode = "spiral"), v)
  z <- pr[[1]]
...
test_that("averaging reports a level that occurred on neither day", {
  ...
  v <- mk_vals()
  av <- dance_profile_rings(list(density_profile_mode = "average"), v)[[1]]
  sp <- dance_profile_rings(list(density_profile_mode = "spiral"), v)[[1]]
  h <- 8
...
test_that("per_day gives one ring per day", {
  v <- mk_vals()
  pr <- dance_profile_rings(list(density_profile_mode = "per_day"), v)
...
test_that("the old checkbox still selects the old behaviour", {
  v <- mk_vals()
  a <- dance_profile_rings(list(density_avg_days = TRUE), v)
  d <- dance_profile_rings(list(density_avg_days = FALSE), v)
...
test_that("a within-day recording behaves the same in every mode", {
  ...
  for (m in c("spiral", "average", "per_day")) {
    pr <- dance_profile_rings(list(density_profile_mode = m), v)

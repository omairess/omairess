...
test_that("P4.3: a hostile column name is fitted as a column, not executed", {
  ...
  fit <- dance_fit_fosr_ols(Y, df, nasty, use_bootstrap = FALSE)
...
test_that("P4.8: the n-basis sweep uses the basis the app would actually fit", {
  ...
  expect_true(grepl("dance_smoothing_basis(axis, nb, input$smooth_method, nb_fourier = nb)",
                    src, fixed = TRUE))

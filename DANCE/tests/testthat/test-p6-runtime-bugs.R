...
test_that("P6.1: the GAM kernel fits, with a factor and a non-syntactic name", {
  ...
  fit <- dance_fit_fosr_gam(Y, df, c("Age (years)", "AGEcategory"))
...
test_that("P6.3: length() of an fd object is 3, so it cannot count harmonics", {
  ...
  for (nh in c(2, 3, 5, 7)) {
    ...
    expect_equal(dance_n_harmonics(p), nh)           # the fix
    expect_equal(ncol(p$scores), nh)
  }
  expect_equal(dance_n_harmonics(NULL), 0L)
  expect_equal(dance_n_harmonics(list()), 0L)
})
...
test_that("P6.3: no display path counts components with length(harmonics)", {
  ...
  expect_true(grepl("nk <- min(dance_n_harmonics(pca_res), length(pca_res$varprop))",
                    src, fixed = TRUE))
  expect_gte(length(gregexpr("dance_n_harmonics(", src, fixed = TRUE)[[1]]), 5L)
})
...
test_that("P6.5: the FoSR observed plot no longer takes the first N rows", {
  ...
  expect_true(grepl("dance_group_ramp(nlevels(df_plot$Color))", src, fixed = TRUE))
})

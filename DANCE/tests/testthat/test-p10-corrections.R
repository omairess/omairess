...
test_that("P10.2: production and the CV use one basis-count rule", {
  ...
  expect_equal(dance_smoothing_nbasis(inp("auto"),   24), 20L)
  expect_equal(dance_smoothing_nbasis(inp("manual"), 24), 20L)
  expect_equal(dance_smoothing_nbasis(inp("none"),   24), 20L)
  expect_equal(dance_smoothing_nbasis(inp("auto"),   16), 16L)   # capped at n_time
  expect_equal(dance_smoothing_nbasis(inp("none"),   16), 14L)   # n_time - 2
  expect_equal(dance_smoothing_nbasis(inp("auto", 2), 24), 4L)   # floor of 4
  expect_equal(dance_smoothing_nbasis(list(), 24), 20L)          # missing input

  # the reviewer's example: 16 time points, 20 requested
  expect_equal(dance_smoothing_nbasis(inp("auto"), 16), 16L)
  expect_equal(max(4, min(20L, 16L - 2L)), 14L)                # the old CV rule
  expect_false(dance_smoothing_nbasis(inp("auto"), 16) == 14L)
})

test_that("P10.2: neither file keeps a private copy of the rule", {
  sm <- code_of("server/20_smoothing.R")
  expect_true(grepl("nb <- dance_smoothing_nbasis(input, n_time)", sm, fixed = TRUE))
  ...
  expect_true(grepl("nb <- dance_smoothing_nbasis(input, n_time)", dg, fixed = TRUE))
  ...
  expect_true(grepl("cv_axis, dance_smoothing_nbasis(input, n_time),", dg, fixed = TRUE))
})

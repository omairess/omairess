...
test_that("Brown-Forsythe detects unequal variances and misses equal ones", {
  ...
  expect_lt(dance_brown_forsythe(het$x, het$g)$p, 0.01)
  expect_gt(dance_brown_forsythe(hom$x, hom$g)$p, 0.05)
})
...
test_that("normality reporting separates severity from Shapiro's p", {
  ...
  mild <- dance_normality_check(rnorm(800)^2 * sign(rnorm(800)), g)
  expect_true(is.finite(mild$skew))
  # severely non-normal
  sev <- dance_normality_check(c(rnorm(790), rnorm(10, 0, 40)), g)
...
test_that("Welch is used automatically when the variances differ", {
  ...
  S <- matrix(d$x, ncol = 1)
  r <- dance_pc_anova_all(S, d$g, eigenvalues = c(1), across_pc_correction = "none")
  o <- r$omnibus[[1]]
...
test_that("the user can force Fisher or Welch", {
  d <- mk(n = c(300, 60), shift = c(0, 0.5), sd = c(1, 4), seed = 7)
  S <- matrix(d$x, ncol = 1)
  rf <- dance_pc_anova_all(S, d$g, across_pc_correction = "none", use_welch = FALSE)
  rw <- dance_pc_anova_all(S, d$g, across_pc_correction = "none", use_welch = TRUE)
...
test_that("the across-component correction actually adjusts the omnibus p's", {
  ...
  raw <- dance_pc_anova_all(S, g, across_pc_correction = "none")
  adj <- dance_pc_anova_all(S, g, across_pc_correction = "holm")
  bon <- dance_pc_anova_all(S, g, across_pc_correction = "bonferroni")
...
test_that("the post-hoc gate runs on the ADJUSTED omnibus p, not the raw one", {
  ...
  raw <- dance_pc_anova_all(S, g, across_pc_correction = "none", posthoc_gate = 0.05)
  bon <- dance_pc_anova_all(S, g, across_pc_correction = "bonferroni", posthoc_gate = 0.05)
...
test_that("components above the gate get no post-hoc table at all", {
  ...
  r <- dance_pc_anova_all(S, g, across_pc_correction = "holm", posthoc_gate = 0.05)
...
test_that("Tukey reproduces TukeyHSD on a balanced equal-variance design", {
  ...
  ph <- dance_pc_posthoc(d$x, d$g, method = "tukey")
...
test_that("Games-Howell differs from Tukey when the variances differ", {
  ...
  tk <- dance_pc_posthoc(d$x, d$g, method = "tukey")
  gh <- dance_pc_posthoc(d$x, d$g, method = "games-howell")
...
test_that("every contrast carries an interval and an effect size", {
  ...
  ph <- dance_pc_posthoc(d$x, d$g, method = "holm")
...
test_that("the number of pairwise tests is k choose 2", {
  for (k in 2:5) {
    ...
    expect_equal(nrow(dance_pc_posthoc(x, g, "holm")), choose(k, 2))
  }
})

# ------------------------------------------------------- eigenvalue separation
test_that("near-tied eigenvalues are flagged as poorly separated", {
  sp <- dance_pc_separation(c(10, 5, 4.9, 1))
...
test_that("one row per participant passes, and no id means no claim", {
  expect_true(dance_pc_independence_check(1:10)$ok)
  expect_false(dance_pc_independence_check(NULL)$checked)
})
...
test_that("the full runner finds a planted effect and rejects the noise", {
  ...
  r <- dance_pc_anova_all(S, g, eigenvalues = c(40, 20, 8, 3, 1),
...
test_that("an untestable component keeps its slot instead of renumbering the rest", {
  ...
  r <- dance_pc_anova_all(S, g, across_pc_correction = "none")
...
test_that("a single-level grouping variable yields nothing rather than nonsense", {
  g <- factor(rep("A", 30))
  expect_null(dance_pc_omnibus(rnorm(30), g))
})

...
test_that("a quantity that cannot exceed 1 loses its leading zero", {
  # APA 7 sec. 6.36
  expect_identical(dance_apa_num(0.427, 3, bounded = TRUE), ".427")
  expect_identical(dance_apa_num(-0.009, 3, bounded = TRUE), "-.009")
  expect_identical(dance_apa_num(0.427, 2, bounded = FALSE), "0.43")
  expect_identical(dance_apa_num(12.3456, 2), "12.35")
  expect_identical(dance_apa_num(NA), "--")
  expect_identical(dance_apa_num(NULL), "--")
  expect_identical(dance_apa_num(Inf), "--")
  # vectorised, because variance tables are built from one call
  expect_identical(dance_apa_num(c(0.1, 0.25), 2, TRUE), c(".10", ".25"))
})

test_that("p-values follow APA 7 sec. 6.42", {
  expect_identical(dance_apa_p(0.0432), "*p* = .043")
  expect_identical(dance_apa_p(0.0004), "*p* < .001")
  expect_identical(dance_apa_p(0.9999), "*p* > .999")
  expect_identical(dance_apa_pval(0.0432), ".043")
  expect_identical(dance_apa_pval(0.0004), "< .001")
  expect_identical(dance_apa_pval(NA), "--")
  # never a leading zero, at any precision
  for (p in c(.5, .05, .005, .0005))
    expect_false(grepl("^0\\.", dance_apa_pval(p, 4)))
})

test_that("F is reported with exact degrees of freedom", {
  expect_identical(dance_apa_F(12.876, 3, 122), "*F*(3, 122) = 12.88")
  expect_identical(dance_apa_F(12.876, 3, 122, 0.0004),
                   "*F*(3, 122) = 12.88, *p* < .001")
  expect_identical(dance_apa_F(NA, 3, 122), "*F* undefined")
})

test_that("a bounded RANGE drops the leading zero at both ends", {
  expect_identical(dance_apa_range(c(0.007, 0.241), 3, bounded = TRUE), ".007 to .241")
  expect_identical(dance_apa_range(c(0.007, 0.241), 3), "0.007 to 0.241")
  expect_identical(dance_apa_range(c(NA, NaN)), "--")
})

test_that("eponymous corrections are written as proper names", {
  expect_identical(dance_apa_method("bonferroni"), "Bonferroni")
  expect_identical(dance_apa_method("holm"), "Holm")
  expect_identical(dance_apa_method("BH"), "Benjamini-Hochberg")
  expect_identical(dance_apa_method("fdr"), "Benjamini-Hochberg")
  expect_identical(dance_apa_method("none"), "no")
  expect_identical(dance_apa_method("something-else"), "something-else")
})

test_that("M and SD are reported together, and drop empty input", {
  expect_identical(dance_apa_M(c(1, 2, 3)), "*M* = 2.00, *SD* = 1.00")
  expect_identical(dance_apa_M(c(NA, NaN)), "--")
})

# ------------------------------------------------------------ the structure --
test_that("a report on an empty session says so and stops", {
  v <- list(data = NULL)
  md <- dance_apa_report(v, list())
...
test_that("repeated observations of one participant are flagged, not hidden", {
  ...
  txt <- paste(dance_apa_report(v, list()), collapse = "\n")
...
test_that("the HTML rendering keeps tables and resolves escaped asterisks", {
  # The escaped-asterisk case is kept even though the app's own name no longer
  # contains one: a column name or factor level supplied by the user can, and
  # that is what the sentinel in dance_apa_html() protects.
  md <- c("# Title", "", "*Measured at T\\*2 on 2026-01-01.*", "",
          "> A note with **bold**.", "",
          "| A | B |", "| :--- | ---: |", "| x | 1 |", "", "---", "",
          "A paragraph with *emphasis* and `code`.")
  h <- paste(dance_apa_html(md, "T"), collapse = "\n")
  ...
  expect_true(grepl("T*2", h, fixed = TRUE))
  expect_false(grepl("T\\2", h, fixed = TRUE))
...
test_that("the HTML escapes angle brackets but keeps the tags the report uses", {
  h <- paste(dance_apa_html(c("*R*<sup>2</sup> and a <script>alert(1)</script>")),
             collapse = "\n")

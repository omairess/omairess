...
test_that("a parameter exactly on a bound is detected, by name and side", {
  expect_equal(dance_bounds_hit(c(mesor = 20, A_sat = 30, tau = 110, b_cos1 = 1, b_sin1 = 2), lo, up),
               "tau (upper)")
  expect_equal(dance_bounds_hit(c(mesor = 20, A_sat = 30, tau = 0.5, b_cos1 = 1, b_sin1 = 2), lo, up),
               "tau (lower)")
})

test_that("an interior fit reports nothing", {
  expect_equal(dance_bounds_hit(c(mesor = 20, A_sat = 30, tau = 14, b_cos1 = 1, b_sin1 = 2), lo, up),
               character(0))
})
...
test_that("infinite bounds are never counted as hit", {
  ...
  expect_false(any(grepl("A_sat",
    dance_bounds_hit(c(mesor = 20, A_sat = 1e9, tau = 14, b_cos1 = 1, b_sin1 = 2), lo, up))))
  expect_equal(dance_bounds_hit(c(a = 1e300), c(a = -Inf), c(a = Inf)), character(0))
})
...
test_that("both bounds on one parameter cannot both fire", {
  ...
  hit <- dance_bounds_hit(c(tau = 5), c(tau = 5), c(tau = 5))
  expect_equal(length(hit), 2)
})

test_that("several parameters can be pinned at once", {
  hit <- dance_bounds_hit(c(mesor = 0, A_sat = 500, tau = 110),
...
test_that("the tolerance is relative, so a big and a small bound bite alike", {
  ...
  expect_equal(dance_bounds_hit(c(tau = 110 * (1 + 1e-9)), c(tau = 0.5), c(tau = 110)),
               "tau (upper)")
  expect_equal(dance_bounds_hit(c(tau = 110 * (1 + 1e-3)), c(tau = 0.5), c(tau = 110)),
               character(0))
  # the same sensitivity at the small end
  expect_equal(dance_bounds_hit(c(tau = 0.5 * (1 + 1e-9)), c(tau = 0.5), c(tau = 110)),
               "tau (lower)")
})

test_that("an unnamed or empty coefficient vector is handled", {
  expect_equal(dance_bounds_hit(c(1, 2, 3), lo, up), character(0))
  expect_equal(dance_bounds_hit(numeric(0), lo, up), character(0))
})

test_that("NA and non-finite coefficients never report a hit", {
  expect_equal(dance_bounds_hit(c(tau = NA_real_), c(tau = 0.5), c(tau = 110)), character(0))
  expect_equal(dance_bounds_hit(c(tau = Inf), c(tau = 0.5), c(tau = 110)), character(0))
})
...
test_that("fits on more than one bound are listed with which bounds", {
  s <- dance_bounds_summary(mk(), subject_ids = letters[1:6])
...
test_that("no fit on a bound gives an empty per-bound table, not an error", {
  s <- dance_bounds_summary(list(character(0), character(0)))

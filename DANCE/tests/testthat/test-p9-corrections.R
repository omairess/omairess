...
test_that("P9.1: a periodic ZERO shift has warping amplitude exactly 0", {
  ...
  a_per <- dance_warp_amplitude(mk(TRUE))
  ...
  expect_equal(dance_warp_amplitude(mk(FALSE))[1:5], c(0, 0.05, 0.10, 0.25, 0.15))
  ...
  a_par <- dance_warp_amplitude(list(method = "parametric", warp_functions = h,
                                   time_points = tp))
...
test_that("P9.1: the definition exists once and all three sites call it", {
  ...
  expect_true(grepl("dance_warp_amplitude <- function(warping_results)", fd, fixed = TRUE))
  fp <- warp_code()
  expect_true(grepl("warp_amplitude <- dance_warp_amplitude(warp_results)", fp, fixed = TRUE))
  ...
  expect_true(grepl("dance_warp_amplitude(warping_results)", pc, fixed = TRUE))
...
test_that("P9.3: the extracted basis builder reproduces production exactly", {
  ...
  same(dance_smoothing_basis(ax(TRUE), 12, "manual"),
       create.fourier.basis(rangeval = tr, nbasis = min(n, 13)))
  same(dance_smoothing_basis(ax(FALSE), 12, "none"),
       create.bspline.basis(rangeval = tr, breaks = ti, norder = 4))
  same(dance_smoothing_basis(ax(FALSE), 12, "manual"),
       create.bspline.basis(rangeval = tr, nbasis = 12))

  # on real elapsed hours a cyclic basis must have a 24-hour period, not the
  # length of the recording
  rt <- seq(8, 8 + 23)
  b <- dance_smoothing_basis(ax(TRUE, TRUE, rt), 12, "manual")
  expect_equal(as.numeric(b$params[1]), 24)

  # and the label names the model the lambda belongs to
  expect_true(grepl("Fourier", dance_basis_label(ax(TRUE), dance_smoothing_basis(ax(TRUE), 12)),
                    fixed = TRUE))
  expect_true(grepl("elapsed hours",
                    dance_basis_label(ax(FALSE, TRUE, rt),
                                    dance_smoothing_basis(ax(FALSE, TRUE, rt), 12)),
                    fixed = TRUE))
})

test_that("P9.3: production and the CV both go through the shared builder", {
  sm <- code_of("server/20_smoothing.R")
  expect_true(grepl("basis   <- dance_smoothing_basis(axis, nb, input$smooth_method)",
                    sm, fixed = TRUE))
  expect_false(grepl("basis   <- create.bspline.basis(rangeval = t_rng, nbasis = nb)",
                     sm, fixed = TRUE))
  dg <- code_of("server/30_diagnostics.R")
  expect_true(grepl("cv_axis     <- dance_smoothing_axis(input, values)", dg, fixed = TRUE))
  expect_true(grepl("basis <- dance_smoothing_basis(cv_axis, nb,", dg, fixed = TRUE))

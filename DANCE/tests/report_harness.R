...
#
# Run:  Rscript tests/report_harness.R          (from the DANCE directory)
...
res1 <- dance_resultants(P$phi1, P$amp1)
res2 <- dance_resultants(P$phi2, P$amp2)
...
cm_each <- Map(dance_commonality, P$r2, P$r2_S, P$r2_C)
...
ram <- dance_rhythm_adjusted_mean(pool_int, "exp_sat", c(pool_A, pool_tau),
...
for (g in names(n_by_group)) {
  ...
  gs <- gs + length(idx)
  rr <- dance_resultants(P$phi1[idx], P$amp1[idx])
  cat(sprintf("\nGroup '%s' (n=%d):\n", g, length(idx)))
...
cat(dance_format_equation(pool_int, "exp_sat", c(pool_A, pool_tau),
...
cat("\n--- Group-specific parameters ---\n")
ga <- dance_group_audit(lab, seq_along(lab))
...
for (g in c(names(n_by_group), "__UNASSIGNED__")) {
  ...
  gram <- dance_rhythm_adjusted_mean(gI, "exp_sat", c(gA, gT), min(tlin), max(tlin), t_off)
  rr <- dance_resultants(P$phi1[idx], P$amp1[idx])
  r2 <- dance_resultants(P$phi2[idx], P$amp2[idx])
  ...
  cat("\n  Fitted equation:\n  ")
  cat(dance_format_equation(gI, "exp_sat", c(gA, gT),
...
for (nm in c("amp1", "tau")) {
  lt <- dance_group_linear_test(P[[nm]], lab_f)
  tr <- dance_group_trend_test(P[[nm]], lab_f)
  cat(sprintf("\n%s:\n", nm))
...
true_ram <- dance_rhythm_adjusted_mean(true_int, "exp_sat", c(true_A, true_tau),
                                     min(tlin), max(tlin), t_off)
...
row("Rayleigh Z (H1)", nrow(P) * res1$r_weighted^2,
    dance_rayleigh(res1$r_unweighted, res1$n)$Z,
    dance_rayleigh(res1$r_unweighted, res1$n)$Z)

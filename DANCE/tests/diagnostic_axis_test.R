...
#
# Run with:   Rscript tests/diagnostic_axis_test.R      (from the DANCE directory)
# ==============================================================================
...
for (real in c(FALSE, TRUE)) {
  for (cyc in c(FALSE, TRUE)) {
    for (meth in c("auto", "manual")) {
      ...
      axis <- e$dance_smoothing_axis(inp, vals)
      nb   <- e$dance_smoothing_nbasis(inp, n_time)
      prod <- e$dance_smoothing_basis(axis, nb, meth)
      # what the "suggest a lambda" observer and the CV path now build
      diag <- e$dance_smoothing_basis(e$dance_smoothing_axis(inp, vals),
                                    e$dance_smoothing_nbasis(inp, n_time), meth)
...
inp <- list(use_real_time = TRUE, is_cyclic = TRUE, smooth_method = "manual",
            n_basis = 12, n_basis_manual = 12)
axis <- e$dance_smoothing_axis(inp, vals)
prod <- e$dance_smoothing_basis(axis, e$dance_smoothing_nbasis(inp, n_time), "manual")
...
chk(e$dance_smoothing_basis(axis, 12, "manual")$nbasis ==
    e$dance_smoothing_basis(axis, 12, "manual", nb_fourier = NULL)$nbasis,
    "nb_fourier = NULL reproduces the production count",
    "nb_fourier = NULL changed the production count")
sw <- e$dance_smoothing_basis(axis, 12, "manual", nb_fourier = 7)
...
for (f in c("server/20_smoothing.R", "server/30_diagnostics.R")) {
  ...
  ax <- grep("using_real_time\\s*<-\\s*!is\\.null|t_full\\s*<-\\s*if\\(using_real_time",
             code, value = TRUE)
  chk(length(ax) == 0,
      paste(f, "derives the time axis only through dance_smoothing_axis()"),
...
chk(grepl("axis\\s*<-\\s*dance_smoothing_axis\\(input, values\\)", prod_src),
    "production smoothing takes its axis from dance_smoothing_axis()",
    "production smoothing no longer calls dance_smoothing_axis()")

...
# dance_* helpers it never defined, the repeated-measures kernel carried
...
#
# Run with:   Rscript tests/export_roundtrip_test.R      (from the DANCE directory)
# ==============================================================================
...
tmp <- file.path(tempdir(), "dance_roundtrip"); dir.create(tmp, showWarnings = FALSE)
...
server <- function(input, output, session) {
  ...
  local({
    ...
    al     <- dance_auto_lambda(raw, time_points, basis)
    lambda <- al$lambda
    ...
    set.seed(202)
    app_fosr <- dance_fit_fosr_ols(smooth_curves, covs, c("Age", "Sex"),
    ...
    values$reg_model <- c(app_fosr, list(dance_settings = list(
  ...
  local({
    time_points <- seq(0, 1, length.out = n_t)
    basis  <- create.bspline.basis(rangeval = c(0, 1), nbasis = 12)
    lambda <- dance_auto_lambda(rm_raw, time_points, basis)$lambda
...
driver <- c(
  ...
  'dance_input_data       <- app$raw',
  'dance_input_covariates <- app$covs',
  ...
  '              n_harmonics = 1, trend_type = "none", use_bounds = FALSE))',
  'set.seed(202)',
  'sc_fosr <- dance_fit_fosr_ols(smooth_curves, covariates, c("Age", "Sex"),',
  '                            use_bootstrap = TRUE, n_boot = 120)',

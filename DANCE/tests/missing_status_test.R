...
#
# Run with:   Rscript tests/missing_status_test.R      (from the DANCE directory)
# Needs no packages.
# ==============================================================================
...
failures <- 0
O <- DANCE_FILL_OBSERVED; I <- DANCE_FILL_INTERPOLATED; E <- DANCE_FILL_EXTRAPOLATED

chk <- function(label, row, expect) {
  got <- as.vector(dance_fill_status(matrix(row, nrow = 1)))
...
st  <- dance_fill_status(m)
tab <- dance_fill_per_subject(st, hours_per_step = 2)
...
if (sum(st == DANCE_FILL_INTERPOLATED) + sum(st == DANCE_FILL_EXTRAPOLATED) !=
    sum(is.na(m))) {
  failures <- failures + 1

...
#
# Run with:   Rscript tests/posthoc_source_test.R      (from the DANCE directory)
# Needs no packages: dance_posthoc_spec() is pure logic over lists.
# ==============================================================================
...
spec <- dance_posthoc_spec(list(posthoc_source = "fanova"), values)
...
spec2 <- dance_posthoc_spec(list(posthoc_source = "fanova"), values2)
...
spec3 <- dance_posthoc_spec(list(posthoc_source = "custom", posthoc_design = "between",
...
spec4 <- dance_posthoc_spec(list(posthoc_source = "custom", posthoc_design = "within",
...
spec5 <- dance_posthoc_spec(list(posthoc_source = "custom", posthoc_design = "between",
...
spec6 <- dance_posthoc_spec(list(posthoc_source = "custom", posthoc_design = "between",
...
spec7 <- dance_posthoc_spec(list(posthoc_source = "custom", posthoc_design = "within",
                               posthoc_subject_var = "Subject",
...
spec8 <- dance_posthoc_spec(list(posthoc_source = "fanova"), list(fanova_results = NULL))
...
check("prefers the row-aligned covariates",
      identical(as.character(dance_rm_column(v, "ID")), c("a", "b", "c", "d")))

v2 <- list(data = matrix(0, 4, 10), covariates = NULL,
           uploaded_data = data.frame(Other = c("a", "b", "c", "d", "e", "f")))
err <- tryCatch({ dance_rm_column(v2, "Other"); NA_character_ },

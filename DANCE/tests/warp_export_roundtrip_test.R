...
#
# Run with:   Rscript tests/warp_export_roundtrip_test.R   (from the DANCE directory)
# ==============================================================================
...
kernels <- c("linear_shift_alignment", "parametric_alignment",
             "landmark_alignment_simple", "dance_landmark_warp")
...
chk(identical(environment(kern$landmark_alignment_simple),
              environment(kern$dance_landmark_warp)),
    "landmark_alignment_simple and dance_landmark_warp share an environment",
    "dance_landmark_warp is not in its caller's scope")
...
if (!is.null(lm_live)) {
  ...
  chk(any(grepl("dance_landmark_warp <- function", emitted)),
      "the exported script carries dance_landmark_warp with its caller",
      "the exported script calls dance_landmark_warp but never defines it")
}

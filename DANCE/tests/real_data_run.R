...
for (nm in c("dance_commonality", "dance_commonality_pct", "dance_zero_amplitude_test",
             "dance_amp_se", "dance_acro_se", "dance_bingham_ci", "phi_to_hours",
             "dance_rhythm_adjusted_mean", "dance_reduced_exp_sat_sse",
             "dance_r_squared", "dance_gaussian_loglik", "%||%"))
...
bsum <- dance_bounds_summary(bl, subject_ids = which(okv))
...
summ <- function(D) {
  r1 <- dance_resultants(D$phi1, D$amp1); r2 <- dance_resultants(D$phi2, D$amp2)
  list(D = D, r1 = r1, r2 = r2,
       int = mean(D$intercept, na.rm = TRUE),
       A = mean(D$A_sat, na.rm = TRUE), tau = mean(D$tau, na.rm = TRUE),
       ram = dance_rhythm_adjusted_mean(mean(D$intercept, na.rm = TRUE), trend,
...
for (g in c("YOUTH","ADULT","MIDDLE_AGE","ELDERLY")) {
  ...
  gs <- gs + length(idx)
  rr <- dance_resultants(Po$phi1[idx], Po$amp1[idx])
  cat(sprintf("\nGroup '%s' (n=%d):\n", g, length(idx)))
...
for (h in 1:2) {
  ...
  ry <- dance_rayleigh(rr$r_unweighted, rr$n)
  cat(sprintf("H%d  n = %d\n", h, rr$n))
...
cat(dance_format_equation(Sn$int, trend, c(Sn$A, Sn$tau),
...
cat("\n--- Group-specific parameters ---\n")
ga <- dance_group_audit(Pn$grp, seq_len(nrow(Pn)))
...
for (g in c("YOUTH","ADULT","MIDDLE_AGE","ELDERLY","__UNASSIGNED__")) {
  ...
  gram <- dance_rhythm_adjusted_mean(gI, trend, c(gA, gT), min(tlin), max(tlin), min(tlin))
  rr <- dance_resultants(Pn$phi1[idx], Pn$amp1[idx]); r2g <- dance_resultants(Pn$phi2[idx], Pn$amp2[idx])
  ...
  cat("\n  Fitted equation:\n  ")
  cat(dance_format_equation(gI, trend, c(gA, gT),
...
for (nm in c("intercept","amp1","amp2","A_sat","tau")) {
  lt <- dance_group_linear_test(Pn[[nm]], lf); tr <- dance_group_trend_test(Pn[[nm]], lf)
  if (is.null(lt)) next
  cat(sprintf("\n%s:\n", nm))
...
row("Rayleigh Z (H1)", fmt1e(nrow(Po) * So$r1$r_weighted^2),
    fmt1e(dance_rayleigh(Sn$r1$r_unweighted, Sn$r1$n)$Z))

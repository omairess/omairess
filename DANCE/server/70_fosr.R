...
# ==========================================================================
  ...
  output$reg_observed_plot <- renderPlotly({
    ...
    if(is_categorical) {
      g <- g + scale_color_manual(values = dance_group_ramp(nlevels(df_plot$Color)))
    } else {
  ...
  output$reg_fitted_plot <- renderPlotly({
    ...
    if(!is.null(mod$gam_obj)) {
      ...
      # no error shown. dance_fosr_design() does the rename and re-applies the
      # fitted factor levels; it is the same code the fit itself uses.
      betas <- mod$beta.hat
      X_new <- tryCatch(dance_fosr_design(mod, pred_df),
                        error = function(e) structure(conditionMessage(e), class = "dance_err"))
      if(inherits(X_new, "dance_err")) {
    ...
    if(any(se_ok)) {
      ...
      if(!all(se_ok))
        p <- p %>% layout(annotations = list(list(
          ...
          xanchor = "left", font = list(size = 11, color = DANCE_ALERT),
    ...
    if(!is.null(ref_pred) && ref_pred %in% input$reg_predictors) {
      ...
      if(is.numeric(orig_vals)) {
        ...
        if(!is.null(mod$gam_obj)) {
          ...
          X_min <- tryCatch(dance_fosr_design(mod, df_min), error = function(e) NULL)
          X_max <- tryCatch(dance_fosr_design(mod, df_max), error = function(e) NULL)

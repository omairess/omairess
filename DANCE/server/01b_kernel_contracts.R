...
DANCE_CONTRACTS <- list(
  ...
  fit_cosinor = list(
    ...
    time_units  = "MODEL time (hours since the chosen origin), never clock time. Clock conversion happens only for display, through dance_model_to_clock().",
    ...
    degenerate  = "constant y gives SST = 0, so R^2 is NA (dance_r_squared), not NaN; a perfect fit floors sigma^2 (dance_gaussian_loglik) so AIC stays finite",
  ...
  fit_cosinor_nonlinear = list(
    ...
    zero_amp    = "the reduced model REFITS tau (dance_reduced_exp_sat_sse). Freezing tau at its full-model value inflated Type-I error from 5% to 14.6%",
  ...
  perform_functional_anova = list(
    ...
    global      = "L2 norm of SSB/n, integrated by trapezoid over the evaluation grid (dance_l2_norm) -- NOT sum of squares over grid points, which scales with grid density",
...
# Look a contract up from the console: dance_contract("fit_cosinor_nonlinear")
dance_contract <- function(name) {
  if (!name %in% names(DANCE_CONTRACTS))
    stop("No contract recorded for '", name, "'. Have: ",
         paste(names(DANCE_CONTRACTS), collapse = ", "))
  x <- DANCE_CONTRACTS[[name]]
  cat("\n", name, "\n", strrep("-", nchar(name)), "\n", sep = "")

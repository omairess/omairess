...
#     PC2 and PC3. dance_pc_separation() quantifies this so a finding on a poorly
...
dance_pc_adjust <- function(p, method) {
...
dance_brown_forsythe <- function(x, g) {
...
dance_normality_check <- function(x, g) {
  ok <- is.finite(x) & !is.na(g)
  x <- x[ok]; g <- droplevels(as.factor(g[ok]))
...
dance_pc_omnibus <- function(scores, g, conf = 0.95) {
...
dance_pc_posthoc <- function(scores, g, method = "holm", conf = 0.95,
  ...
  out$p_adj <- dance_pc_adjust(out$p_raw, method)
  out$method <- method
...
dance_pc_separation <- function(eigenvalues) {
...
dance_pc_independence_check <- function(subject_ids) {
...
dance_pc_anova_all <- function(scores, g, eigenvalues = NULL, varprop = NULL,
                             posthoc_method = "holm",
  ...
  for (j in seq_len(k)) {
    ...
    omni[j]  <- list(dance_pc_omnibus(scores[, j], g, conf))
    assum[j] <- list(dance_brown_forsythe(scores[, j], g))
    norm[j]  <- list(dance_normality_check(scores[, j], g))
  }
  ...
  p_omni_adj[keep] <- dance_pc_adjust(p_omni[keep],
  ...
  for (j in seq_len(k)) {
    ...
    ph[j] <- list(dance_pc_posthoc(scores[, j], g, posthoc_method, conf,
  ...
  list(k = k,
       ...
       separation = if (!is.null(eigenvalues)) dance_pc_separation(eigenvalues[seq_len(k)]) else NULL,
       varprop = varprop,
       independence = dance_pc_independence_check(subject_ids),
       levels = levels(g))
}
...
# other's correction.
dance_warp_params <- function(warping_results, time_points = NULL) {
  ...
  amp <- if (exists("dance_warp_amplitude", mode = "function"))
    dance_warp_amplitude(warping_results) else NULL

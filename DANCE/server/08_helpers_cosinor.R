...
#             wrongly, or absent:
#
#   dance_format_equation()   ONE fitted-equation renderer. The pooled and group
...
#   dance_resultants()        Returns the unweighted resultant (what the Rayleigh
...
#   dance_rayleigh()          Rayleigh Z and p from the UNWEIGHTED resultant,
#                           with the standard small-sample correction rather
#                           than the bare exp(-Z). (audit 1.2)
#   dance_commonality()       unique_S + unique_C + shared == total R2 exactly.
#                           Replaces two overlapping marginal R2s that summed
#                           to 124.7%. (audit 1.3)
#   dance_rhythm_adjusted_mean()
...
#   dance_amp_se(), dance_acro_se()
#                           Delta-method SEs that use the full covariance block,
#                           not sqrt(se_c^2 + se_s^2)/sqrt(2). (audit, extra c)
#   dance_bingham_ci()        Elliptical joint amplitude-acrophase region.
#                           (audit 2.5)
#   dance_zero_amplitude_test()
...
dance_round2 <- function(x, digits = 2) {
  z <- abs(x) * 10^digits
...
fmtn <- function(x, digits = 2, na = "NA") {
  out <- rep(na, length(x))
  ok <- is.finite(x)
  if (any(ok)) out[ok] <- formatC(dance_round2(x[ok], digits), format = "f", digits = digits)
  out
}
...
dance_phase_convention <- function(period = 24, harmonic = 1) {
  if (harmonic <= 1) {
...
# structural fix.
dance_resultants <- function(phi, amplitude = NULL) {
  ok <- is.finite(phi)
...
# the leading term.
dance_rayleigh <- function(r_unweighted, n) {
...
dance_commonality <- function(r2_full, r2_S_only, r2_C_only) {
...
dance_commonality_pct <- function(cm) {
...
dance_trend_value <- function(trend_type, coefs, t, t_offset = 0) {
...
dance_rhythm_adjusted_mean <- function(mesor, trend_type, trend_coefs,
  ...
  f <- function(t) dance_trend_value(trend_type, trend_coefs, t, t_offset)
  area <- tryCatch(
...
dance_harmonic_window_mean <- function(beta_cos, beta_sin, period, t_min, t_max) {
  n_h <- length(beta_cos)
...
dance_amp_se <- function(beta_cos, beta_sin, V) {
...
dance_acro_se <- function(beta_cos, beta_sin, V) {
...
dance_bingham_ci <- function(beta_cos, beta_sin, V, n, n_params, level = 0.95,
...
dance_zero_amplitude_test <- function(ss_resid_full, ss_resid_trend_only,
...
dance_format_equation <- function(mesor, trend_type = "none", trend_coefs = NULL,
...
dance_group_audit <- function(group_labels, subject_ids, min_n = 3) {
...
dance_akaike_table <- function(aicc_by_model) {
...
dance_check_bounds <- function(fitted_values, lower = NA, upper = NA) {
...
dance_group_linear_test <- function(x, g, conf = 0.95) {
...
dance_group_trend_test <- function(x, g, conf = 0.95) {
  ok <- is.finite(x) & !is.na(g)
  x <- x[ok]; g <- droplevels(as.factor(g[ok]))
  k <- nlevels(g)
  if (k < 3) return(NULL)
...
dance_ww_assumption <- function(r_bar, kappa = NA) {
...
dance_clock_label <- function(t, period = 24, show_day = TRUE, with_minutes = TRUE) {
...
dance_clock_ticks <- function(t_range, period = 24, target_n = 12) {
  ...
  list(vals = vals,
       text = dance_clock_label(vals, period, show_day = FALSE,
                              with_minutes = step < 1),
       step = step)
}
...
dance_bounds_hit <- function(coefs, lower = NULL, upper = NULL, tol = 1e-6) {
  nm <- names(coefs)
...
dance_bounds_summary <- function(bounds_list, subject_ids = NULL) {
...
# dance_curve_peak_clock() below reports that separately; conflating the two is
...
dance_clock_origin <- function(mod) {
...
dance_acrophase_clock <- function(phi_rad = NULL, hours = NULL, period = 24,
...
dance_acrophase_label <- function(phi_rad = NULL, hours = NULL, period = 24,
                                harmonic = 1, clock_origin = 0, all = TRUE) {
  a <- dance_acrophase_clock(phi_rad, hours, period, harmonic, clock_origin)
  if (is.null(a)) return(NA_character_)
  vapply(seq_along(a$hours), function(i) {
    if (!is.finite(a$hours[i])) return(NA_character_)
    if (!all || harmonic <= 1) return(dance_clock_label(a$hours[i], period, show_day = FALSE))
    paste(dance_clock_label(a$all_hours[[i]], period, show_day = FALSE), collapse = " / ")
  }, character(1))
}
...
# NOTE: this calls dance_rhythm_from_coefs() from server/07_helpers_circular.R.
# The app sources 07 before 08, so it is always in scope there; a script or test
# using this function must source 07 as well.
dance_curve_peak_clock <- function(coefs, mod, n_grid = 2001) {
  tv <- mod$time_vec
  ...
  y <- dance_rhythm_from_coefs(coefs, t_grid, period, nh, trend,
                             include_trend = TRUE, t_offset = mod$t_offset %||% 0)
  if (!any(is.finite(y))) return(NULL)
  o <- dance_clock_origin(mod)
...
dance_value_at <- function(coefs, mod, t_model) {
  if (is.null(coefs) || !is.finite(t_model)) return(NA_real_)
  v <- dance_rhythm_from_coefs(coefs, t_model, mod$period %||% 24,
...
# than an absent one.
dance_reduced_exp_sat_sse <- function(y, time, t_offset, start, lower = NULL, upper = NULL) {
...
dance_r_squared <- function(ss_resid, ss_total) {
...
dance_gaussian_loglik <- function(ss_resid, n, y_scale = NULL) {
...
DANCE_TREND_TYPES <- c("none", "linear", "log", "exp_sat")

# Free parameters each trend contributes, beyond the MESOR.
dance_trend_npar <- function(trend) {
  switch(trend, "none" = 0L, "linear" = 1L, "log" = 1L, "exp_sat" = 2L, 0L)
}

dance_trend_label <- function(trend) {
  switch(trend,
         "none"    = "no trend",
         "linear"  = "linear trend",
...
dance_harmonic_label <- function(k) {
  k <- as.integer(k)
  if (!is.finite(k) || k < 1) return("H0")
  if (k == 1) "H1" else sprintf("H1-H%d", k)
}

dance_model_label <- function(trend, k) paste0(trend, " + ", dance_harmonic_label(k))
...
# parameters.
dance_model_npar <- function(trend, k) 1L + dance_trend_npar(trend) + 2L * as.integer(k)
...
dance_cosinor_model_set <- function(n_harmonics) {
  k_max <- max(1L, as.integer(n_harmonics))
  out <- list()
  for (tt in DANCE_TREND_TYPES) for (k in seq_len(k_max))

...
#
# DANCE_RMFANOVA_DEFAULT is the curated set: the two permutation schemes that came
# out closest to nominal with full power. Everything else is available but must
# be asked for, and is labelled.
DANCE_RMFANOVA_DEFAULT <- c("Cn_P1", "Dn_P1", "En_P1")
DANCE_RMFANOVA_INFLATED <- c("En_P2", "En_B2")
DANCE_RMFANOVA_NOPOWER  <- c("Cn_P2", "Cn_B2")
...
dance_rm_design <- function(curves, subject_id, rm_factor) {
...
dance_rmfanova_global <- function(curves, subject_id, rm_factor,
  ...
  d <- dance_rm_design(curves, subject_id, rm_factor)
  ...
  list(ok = TRUE,
       stats = fit$test_stat,
       p_values = pv,
       p_default = pv[intersect(DANCE_RMFANOVA_DEFAULT, names(pv))],

...
# ==========================================================================
  ...
  generate_analysis_code <- function(full = TRUE) {
    ...
    add(sprintf("dance_recorded_versions <- c(%s)", paste(.have, collapse = ", ")))
    add("dance_check_env <- function(recorded) {")
    ...
    add("  invisible(NULL)")
    add("}")
    add("dance_check_env(dance_recorded_versions)")
    add("")
    ...
    #   perform_functional_anova -> dance_l2_norm
    #   perform_rm_fanova        -> dance_l2_norm, dance_rmfanova_global, dance_rm_design
    #   fit_cosinor              -> 11 helpers, via fit_cosinor_nonlinear
    # so the exported cosinor script died at "could not find function
    # dance_r_squared" on the first subject, and the fANOVA script at
    # dance_l2_norm. A script that stops on line one is not a reproducibility
    ...
    dance_fn_order <- function(nm, seen = character(0), stack = character(0)) {
      ...
      for (d in Filter(dance_is_app_fn, g)) seen <- dance_fn_order(d, seen, stack)
      c(seen, nm)
    }

    emit_kernel <- function(nm) {
      for (f in dance_fn_order(nm)) {
    ...
    # P3.5: the repeated-measures kernel calls dance_rmfanova_global(), which
    ...
    if(!is.null(values$data)) {
      ...
      add("#")
      add("# P3.8: if an object called dance_input_data is already in the session,")
      ...
      add("if (exists('dance_input_data')) {")
      ...
      if(!is.null(values$covariates) && !is.null(values$reg_model)) {
        ...
        add("if (exists('dance_input_covariates')) {")
        add("  covariates <- as.data.frame(dance_input_covariates)")
        add("} else {")
    ...
    if(!is.null(values$smooth_fit_metrics)) {
      ...
      if(smooth_method == "none") {
        ...
        # lambda by GCV (dance_auto_lambda); the script now states the value that
        # search returned and offers the search itself as a re-run.
        add("# Smoothing method: Automatic, lambda chosen by GCV (dance_auto_lambda)")
        ...
        emit_kernel("dance_auto_lambda")
        add("# lambda <- dance_auto_lambda(data_matrix, time_points, basis)$lambda")
        add("")
    ...
    # new. Settings come from the fit itself (dance_settings), never from the
    # live widgets, so the script matches the model that is on screen.
    if(!is.null(values$harmonic_model)) {
      hm <- values$harmonic_model
      st <- hm$dance_settings
      add("# -----------------------------------------------------------------------------")
      ...
      # P3.5: emit_kernel() pulls in fit_cosinor_nonlinear and the eleven dance_*
      ...
      # failed at the first fit with "could not find function dance_r_squared".
    ...
    if(!is.null(values$reg_model) && !is.null(values$reg_model$dance_settings)) {
      st <- values$reg_model$dance_settings
      add("# -----------------------------------------------------------------------------")
      ...
      if(identical(st$method, "OLS_nosmooth")) {
        ...
        emit_kernel("dance_fit_fosr_ols")
        ...
        add(sprintf("fosr_fit <- dance_fit_fosr_ols(Y, df_reg, fosr_predictors,\n                             use_bootstrap = %s, n_boot = %s)",
        ...
        emit_kernel("dance_fit_fosr_gam")
        add(sprintf("fosr_fit <- dance_fit_fosr_gam(Y, df_reg, fosr_predictors, k = %d)",
    ...
    content = function(file) {
      ...
      if(!is.null(mod$pop_mean_fit)) {
        ...
        cat(sprintf("Acrophase: %s clock  (%.4f rad, %.2f h model-elapsed, origin %s)\n",
                    dance_acrophase_label(hours = pop$mean_acrophase_time,
      ...
      cat(sprintf("Acrophase (H1): arithmetic mean %s clock (%.2f h model-elapsed)\n",
                  dance_acrophase_label(hours = mean(params$acrophase_time_1),
                                      period = mod$period, harmonic = 1,
                                      clock_origin = dance_clock_origin(mod), all = FALSE),

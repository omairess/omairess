...
# DANCE_DENSITY_COLORS in the polar density tab. The same age band was a different
...
# two figures on the same screen. dance_group_colors() keys on the LEVEL NAME
...
# same colour for roughly 1 in 12 male readers. DANCE_PALETTE_VARIANT switches
# between:
#
...
DANCE_PALETTE_VARIANT <- "supplied"
...
# alongside a dash pattern; see DANCE_GROUP_DASH.
.DANCE_LIGHTS <- c("#8fd7d7", "#ff8ca1", "#bdd373", "#ffcd8e")

DANCE_GROUP_COLORS <- .DANCE_MEDIUMS[[
  if (DANCE_PALETTE_VARIANT %in% names(.DANCE_MEDIUMS)) DANCE_PALETTE_VARIANT else "supplied"
]]
...
DANCE_GROUP_COLORS_EXTRA <- .DANCE_LIGHTS
...
# share one.
DANCE_GROUP_DASH <- c("solid", "dash", "dot", "dashdot",
                    "longdash", "longdashdot", "dash", "dot")

# The full ramp a caller may draw from, validated prefix first.
dance_group_ramp <- function(n) {
  full <- c(DANCE_GROUP_COLORS, DANCE_GROUP_COLORS_EXTRA)
...
dance_component_colors <- function(n) dance_group_ramp(n)
...
#
#     cols <- dance_group_colors(levels(g))
...
dance_group_colors <- function(levels, all_levels = NULL) {
  lv <- as.character(levels)
  lv <- lv[!is.na(lv) & nzchar(lv)]
  ref <- if (!is.null(all_levels)) {
    r <- as.character(all_levels); r[!is.na(r) & nzchar(r)]
  } else lv
  ref <- sort(unique(ref))   # the same order dance_group_reference() produces
  pal <- dance_group_ramp(length(ref))
  names(pal) <- ref
  out <- pal[unique(lv)]
  names(out) <- unique(lv)
...
dance_group_reference <- function(levels, all_levels = NULL) {
...
#
# This originally took its reference order from names(dance_group_colors(...)),
...
dance_group_dashes <- function(levels, all_levels = NULL) {
  ref <- dance_group_reference(levels, all_levels)
  lv <- unique(as.character(levels))
  lv <- lv[!is.na(lv) & nzchar(lv)]
  idx <- match(lv, ref)
  d <- DANCE_GROUP_DASH[((idx - 1) %% length(DANCE_GROUP_DASH)) + 1]
  d[is.na(idx)] <- "solid"
  names(d) <- lv
  d
}
...
dance_needs_dash <- function(n) n > length(DANCE_GROUP_COLORS)
...
DANCE_SERIES1 <- unname(DANCE_GROUP_COLORS[1])
DANCE_SERIES2 <- unname(DANCE_GROUP_COLORS[2])

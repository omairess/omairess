...
# ==============================================================================
  ...
  output$download_apa_md <- downloadHandler(
    ...
    content  = function(file)
      writeLines(dance_apa_report(values, input, dance_report_title(input)), file,
                 useBytes = TRUE)
  )
  ...
  output$download_apa_html <- downloadHandler(
    ...
    content  = function(file) {
      md <- dance_apa_report(values, input, dance_report_title(input))
      writeLines(dance_apa_html(md, dance_report_title(input) %||%

...
# ==========================================================================
  ...
  observeEvent(input$run_cv_analysis, {
    ...
    tryCatch({
      ...
      cv_axis     <- dance_smoothing_axis(input, values)
      ...
      values$cv_results <- list(
        ...
        basis_label = tryCatch(
          dance_basis_label(cv_axis, dance_smoothing_basis(
            cv_axis, dance_smoothing_nbasis(input, n_time),
  ...
  output$smoothing_comparison_summary <- renderPrint({
    ...
    cat("    (dance_auto_lambda), per run, on this data. That is what the app\n")
  ...
  observeEvent(input$use_diagnostic_lambda, {
    ...
    # production goes through dance_smoothing_axis(), which uses elapsed hours
    ...
    al <- tryCatch(dance_auto_lambda(dat, axis$t_full, basis,
  ...
  observeEvent(input$run_nbasis_analysis, {
    ...
    make_basis <- function(nb) {
      if (cyclic) dance_smoothing_basis(axis, nb, input$smooth_method, nb_fourier = nb)
      else        dance_smoothing_basis(axis, nb, "manual")
    }
    ...
    withProgress(message = if(auto)
      ...
      for(bi in seq_along(nb_seq)) {
        ...
        if(auto) {
          ...
          al <- tryCatch(dance_auto_lambda(data_mat, axis$t_full, basis,
                                         min_points_needed = if(cyclic) 3 else 4,
                                         n_grid = 12),
                         error = function(e) NULL)

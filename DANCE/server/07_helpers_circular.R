...
dance_hour_to_theta <- function(hours, period = 24) {
  (270 - (360 / period) * (hours %% period)) %% 360
}

# ...and back, for hover text and tick labels.
dance_theta_to_hour <- function(theta_deg, period = 24) {
...
dance_bw_to_kappa <- function(bw_hours, period = 24) {
...
dance_density_contrast <- function(dens) {
...
dance_circular_density <- function(hours, bw_hours, period = 24, n_grid = 361,
                                 weights = NULL) {
  a <- (hours %% period) * 2 * pi / period
  ok <- is.finite(a)
  a <- a[ok]
  ...
  kappa <- dance_bw_to_kappa(bw_hours, period)
...
dance_circular_summary <- function(hours, period = 24, weights = NULL) {
  a <- (hours %% period) * 2 * pi / period
  ok <- is.finite(a)
  a <- a[ok]
  if (!length(a)) return(NULL)
...
dance_night_arcs <- function(dusk = 18, dawn = 6, period = 24) {
...
dance_wrap_to_clock <- function(cum_hours, period = 24) {
...
dance_close_ring <- function(hours, values) {
...
dance_band_ring <- function(hours, lo, hi) {
...
# ==============================================================================
dance_rhythm_from_coefs <- function(coefs, time_vec, period, n_harmonics,
...
# and nothing is repeated to close a seam. dance_close_ring() sorts by clock hour,
...
dance_open_path <- function(hours, values) {
  ok <- is.finite(hours) & is.finite(values)
  hours <- hours[ok]; values <- values[ok]
  if (length(hours) < 2) return(NULL)
...
dance_band_path <- function(hours, lo, hi) {
  ok <- is.finite(hours) & is.finite(lo) & is.finite(hi)
  hours <- hours[ok]; lo <- lo[ok]; hi <- hi[ok]
  if (length(hours) < 2) return(NULL)
...
# ==============================================================================
dance_night_gradient <- function(dusk = 23, dawn = 7, period = 24, n = 72,

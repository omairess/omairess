...
DANCE_DENSITY_COLORS <- c(DANCE_GROUP_COLORS, DANCE_GROUP_COLORS_EXTRA)
DANCE_DENSITY_DASH   <- DANCE_GROUP_DASH
...
dance_density_data <- function(input, values) {
  ...
  hours <- (as.numeric(params[[acro_col]]) + dance_clock_origin(mod)) %% (period / h)
...
# note says so.
dance_fit_rings <- function(input, values) {
  mod <- values$harmonic_model
  if (is.null(mod)) return(NULL)
  ...
  if (is_mixed && with_trend && length(out)) {
    ...
    if (!is.null(g1$mean_coefs)) {
      ...
      base_pred <- if (!is.null(ref$parent)) {
        ...
        dance_rhythm_from_coefs(mod$group_fits[[ref$parent]]$mean_coefs, t_abs,
...
dance_profile_rings <- function(input, values) {
  if (is.null(values$smooth_data) || is.null(values$time_clock)) return(NULL)
  Y <- values$smooth_data
  cum <- dance_cumulative_hours(values$time_labels)
  if (is.null(cum) || length(cum) != ncol(Y)) return(NULL)
  # dance_cumulative_hours() returns ELAPSED hours from the first column (0, 1.5,
  ...
  # The clock hour of each column is what dance_clock_hours() already parsed; cum
  # is only needed for the day index.
  clock <- dance_clock_hours(values$time_labels)
...
output$harmonic_density_plot <- renderPlotly({
  ...
  if (identical(mode, "fit")) {
    fr <- dance_fit_rings(input, values)
    ...
    shape <- if (fit_closed) dance_close_ring else dance_open_path
    ribbon <- if (fit_closed) dance_band_ring else dance_band_path
    ...
    pr <- dance_profile_rings(input, values)
    ...
    for (nm in names(pr)) {
      ...
      shape  <- if (is_spiral) dance_open_path else dance_close_ring
      ribbon <- if (is_spiral) dance_band_path else dance_band_ring
    ...
    dd <- dance_density_data(input, values)
    ...
    bw <- if (identical(input$density_bw_mode %||% "auto", "manual") && !is.null(input$density_bw))
      input$density_bw else dance_default_bandwidth(dd$hours, period, weights)

    dens <- lapply(groups, function(idx)
      dance_circular_density(dd$hours[idx], bw, period, weights = weights[idx]))
    ...
    for (i in seq_along(groups)) {
      ...
      if (isTRUE(input$density_show_mean)) {
        s <- dance_circular_summary(dd$hours[groups[[i]]], period, weights[groups[[i]]])
  ...
  if (identical(night_style, "gradient")) {
    gr <- dance_night_gradient(dusk, dawn, period, n = 180)
    if (!is.null(gr)) {
      ...
      p <- add_trace(p, type = "barpolar",
                     ...
                     base = night_base,
                     theta = dance_hour_to_theta(gr$hour, period),
    ...
    for (arc in dance_night_arcs(dusk, dawn, period)) {
      hrs <- seq(arc[1], arc[2], length.out = 60)
      p <- add_trace(p, type = "scatterpolar", mode = "lines",
                     r = c(rep(1.10, length(hrs)), 0),
                     theta = c(dance_hour_to_theta(hrs, period), dance_hour_to_theta(hrs[1], period)),
  ...
  .ring_pal <- dance_group_colors(.ring_levels)
  .ring_dsh <- dance_group_dashes(.ring_levels)
  .ring_col <- function(x) {
  ...
  for (i in seq_along(bands)) {
    ...
    if (ci_style %in% c("shaded", "both")) {
      p <- add_trace(p, type = "scatterpolar", mode = "lines",
                     r = b$r, theta = dance_hour_to_theta(b$hours, period),
    ...
    if (ci_style %in% c("dotted", "both") && !is.null(b$edges)) {
      for (k in seq_along(b$edges)) {
        e <- b$edges[[k]]
        p <- add_trace(p, type = "scatterpolar", mode = "lines",
                       r = e$r, theta = dance_hour_to_theta(e$hours, period),
  ...
  if (do_fill) {
    for (i in seq_along(rings)) {
      ...
      p <- add_trace(p, type = "scatterpolar", mode = "lines",
                     r = fr_r, theta = dance_hour_to_theta(fr_hours, period),
                     fill = "toself",
  ...
  for (i in seq_along(rings)) {
    ...
    col <- .ring_col(nm[i])
    p <- add_trace(p, type = "scatterpolar", mode = "lines",
                   r = rg$r, theta = dance_hour_to_theta(rg$hours, period),
  ...
  if (!is.null(pts)) {
    for (i in seq_along(pts$groups)) {
      ...
      p <- add_trace(p, type = "scatterpolar", mode = "markers",
                     ...
                     theta = dance_hour_to_theta(pts$hours[idx], period),
  ...
  for (i in seq_along(means)) {
    ...
    p <- add_trace(p, type = "scatterpolar", mode = "lines+markers",
                   ...
                   theta = rep(dance_hour_to_theta(s$mean_hour, period), 2),
  ...
  plotly::layout(p,
    polar = list(
      ...
      angularaxis = list(
        tickmode = "array",
        tickvals = dance_hour_to_theta(tick_h, period),
        ticktext = hour_lab,
...
output$harmonic_density_note <- renderText({
  ...
  weights <- if (isTRUE(input$density_weight_amp)) dd$amps else NULL
  bw <- if (identical(input$density_bw_mode %||% "auto", "manual") && !is.null(input$density_bw))
    input$density_bw else dance_default_bandwidth(dd$hours, dd$period, weights)
  s <- dance_circular_summary(dd$hours, dd$period, weights)
  ...
  paste0(
    ...
    {
      dens_all <- dance_circular_density(dd$hours, bw, dd$period, weights = weights)
      ct <- dance_density_contrast(dens_all)

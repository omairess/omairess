...
output$save_session <- downloadHandler(
  filename = function() sprintf("dance_session_%s.rds", format(Sys.time(), "%Y%m%d_%H%M%S")),
  content = function(file) {
    ...
    saveRDS(list(
      format       = DANCE_SESSION_FORMAT,
      app          = "DANCE",
      saved_at     = Sys.time(),
      r_version    = R.version.string,
      packages     = dance_package_versions(),
      values       = state,
      settings     = settings,
...
observeEvent(input$load_session, {
  ...
  if (!is.list(saved) || is.null(saved$format) || is.null(saved$values)) {
    showNotification("That .rds is not an DANCE session file.", type = "error", duration = 10)
    return()
  }
  if (saved$format > DANCE_SESSION_FORMAT) {
    showNotification(
      sprintf("This session was written by a newer version of the app (format %d > %d). Update before opening it.",
              saved$format, DANCE_SESSION_FORMAT),

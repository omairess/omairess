...
DANCE_FILL_OBSERVED     <- 0L
DANCE_FILL_INTERPOLATED <- 1L
DANCE_FILL_EXTRAPOLATED <- 2L

DANCE_FILL_LABELS <- c("Observed", "Interpolated", "Extrapolated")
...
dance_fill_status <- function(data_mat) {
  data_mat <- as.matrix(data_mat)
  n <- nrow(data_mat); p <- ncol(data_mat)
  obs <- !is.na(data_mat)
  status <- matrix(DANCE_FILL_OBSERVED, n, p,
  ...
  for (i in seq_len(n)) {
    ...
    if (!length(which_obs)) {          # nothing at all: every value is invented
      status[i, ] <- DANCE_FILL_EXTRAPOLATED
      next
    }
    inside <- seq_len(p) >= min(which_obs) & seq_len(p) <= max(which_obs)
    status[i, !obs[i, ] &  inside] <- DANCE_FILL_INTERPOLATED
    status[i, !obs[i, ] & !inside] <- DANCE_FILL_EXTRAPOLATED
  }
  status
}
...
dance_fill_per_subject <- function(status, hours_per_step = NA_real_) {
  n <- nrow(status)
  ...
  out <- data.frame(
    Row          = seq_len(n),
    Observed     = rowSums(status == DANCE_FILL_OBSERVED),
    Interpolated = rowSums(status == DANCE_FILL_INTERPOLATED),
    Extrapolated = rowSums(status == DANCE_FILL_EXTRAPOLATED),
    stringsAsFactors = FALSE
  )
  out$`% filled`     <- round(100 * (out$Interpolated + out$Extrapolated) / ncol(status), 1)
  out$`Longest gap`  <- vapply(seq_len(n), function(i)
    longest_run(status[i, ] == DANCE_FILL_INTERPOLATED), integer(1))
  out$`First observed` <- vapply(seq_len(n), function(i) {
    w <- which(status[i, ] == DANCE_FILL_OBSERVED); if (length(w)) w[1] else NA_integer_
  }, integer(1))
  out$`Last observed` <- vapply(seq_len(n), function(i) {
    w <- which(status[i, ] == DANCE_FILL_OBSERVED); if (length(w)) w[length(w)] else NA_integer_
  }, integer(1))
...
dance_fill_headline <- function(status) {
  tot <- length(status)
  sprintf("%d observed (%.0f%%), %d interpolated within the observed range (%.0f%%), %d extrapolated beyond it (%.0f%%)",
          sum(status == DANCE_FILL_OBSERVED),     100 * mean(status == DANCE_FILL_OBSERVED),
          sum(status == DANCE_FILL_INTERPOLATED), 100 * mean(status == DANCE_FILL_INTERPOLATED),
          sum(status == DANCE_FILL_EXTRAPOLATED), 100 * mean(status == DANCE_FILL_EXTRAPOLATED))
}
...
dance_row_labels <- function(values) {
...
# a reader can judge.
dance_hours_per_step <- function(values) {
  cum <- tryCatch(dance_cumulative_hours(values$time_labels), error = function(e) NULL)

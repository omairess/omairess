...
output$missing_map <- renderPlotly({
  ...
  scale <- list(
    list(0.000, unname(DANCE_FILL_COLORS["Observed"])),
    list(0.333, unname(DANCE_FILL_COLORS["Observed"])),
    list(0.333, unname(DANCE_FILL_COLORS["Interpolated"])),
    list(0.667, unname(DANCE_FILL_COLORS["Interpolated"])),
    list(0.667, unname(DANCE_FILL_COLORS["Extrapolated"])),
    list(1.000, unname(DANCE_FILL_COLORS["Extrapolated"])))
...
output$missing_headline <- renderText({
  req(values$fill_status)
  st <- values$fill_status
  hps <- dance_hours_per_step(values)
  extrap_rows <- sum(rowSums(st == DANCE_FILL_EXTRAPOLATED) > 0)
  half_rows   <- sum(rowMeans(st != DANCE_FILL_OBSERVED) > 0.5)

  paste0(
    dance_fill_headline(st), "\n",
    ...
    if (is.finite(hps)) sprintf(" (up to %.0f h past the last measurement)",
                                max(rowSums(st == DANCE_FILL_EXTRAPOLATED)) * hps) else "",
    ".\n",
    if (half_rows > 0)
...
output$missing_table <- renderDT({
  ...
  st  <- values$fill_status
  hps <- dance_hours_per_step(values)
  tab <- dance_fill_per_subject(st, hps)
  tab$Row <- dance_row_labels(values)
  names(tab)[1] <- "Subject"
  ...
  datatable(tab, rownames = FALSE,
    ...
    formatStyle("Extrapolated",
                background = styleInterval(0, c("", DANCE_FILL_COLORS[["Extrapolated"]])),
...
output$missing_subject_ui <- renderUI({
  ...
  sel <- if (!is.null(st)) which.max(rowSums(st != DANCE_FILL_OBSERVED)) else 1L
...
output$missing_curve <- renderPlotly({
  ...
  x <- dance_cumulative_hours(values$time_labels)
  ...
  for (state in c("Observed", "Interpolated", "Extrapolated")) {
    k <- which(st == (match(state, DANCE_FILL_LABELS) - 1L))
    if (!length(k)) next
    ...
    p <- add_trace(p, x = x[k], y = y, type = "scatter", mode = "markers",
                   ...
                   marker = list(
                     ...
                     color  = if (state == "Observed") DANCE_FILL_COLORS[["Observed"]] else "rgba(0,0,0,0)",
                     symbol = if (state == "Observed") "circle" else
                              if (state == "Interpolated") "circle-open" else "x",
                     line   = list(width = 2, color = unname(DANCE_FILL_COLORS[[state]]))),
                   hoverinfo = "text",
                   text = sprintf("%s<br>%s<br>%.2f", labs[k], state, y))
  }
  n_obs <- sum(st == DANCE_FILL_OBSERVED)
  plotly::layout(p,
    title = list(text = sprintf("%s — %d of %d points measured",
                                dance_row_labels(values)[i], n_obs, length(st)),
                 font = list(size = 14)),
...
output$export_fill_status_csv <- downloadHandler(
  ...
  content = function(file) {
    req(values$fill_status)
    st <- values$fill_status
    df <- as.data.frame(matrix(DANCE_FILL_LABELS[as.vector(st) + 1L], nrow = nrow(st)))
    labs <- values$time_labels
    names(df) <- if (!is.null(labs) && length(labs) == ncol(st)) labs else paste0("T", seq_len(ncol(st)))
    df <- cbind(Subject = dance_row_labels(values), df)

...
#
# APA 7 formatting is implemented in dance_apa_* below: no leading zero on a
...
dance_apa_num <- function(x, digits = 2, bounded = FALSE) {
...
dance_apa_p <- function(p, digits = 3) {
  v <- dance_apa_pval(p, digits)
  if (identical(v, "--")) return("--")
  if (substr(v, 1, 1) == "<" || substr(v, 1, 1) == ">") paste0("*p* ", v)
  else paste0("*p* = ", v)
}

dance_apa_pval <- function(p, digits = 3) {
...
dance_apa_F <- function(Fv, df1, df2, p = NULL) {
  if (!is.finite(suppressWarnings(as.numeric(Fv)))) return("*F* undefined")
  out <- sprintf("*F*(%s, %s) = %s", format(df1), format(df2), dance_apa_num(Fv, 2))
  if (!is.null(p)) out <- paste0(out, ", ", dance_apa_p(p))
  out
}
...
# why it shipped.
dance_apa_M <- function(x, digits = 2, bounded = FALSE) {
  x <- x[is.finite(x)]
  if (!length(x)) return("--")
  sprintf("*M* = %s, *SD* = %s", dance_apa_num(mean(x), digits, bounded),
...
dance_apa_range <- function(x, digits = 2, bounded = FALSE) {
  x <- x[is.finite(x)]
  if (!length(x)) return("--")
  sprintf("%s to %s", dance_apa_num(min(x), digits, bounded),
          dance_apa_num(max(x), digits, bounded))
}
...
dance_md_table <- function(df, align = NULL) {
...
# what was on screen.
dance_apa_report <- function(values, input, title = NULL) {
  L <- character(0)
  ...
  blank()
  add("*Generated by DANCE on ", format(Sys.time(), "%Y-%m-%d %H:%M"), ".*")
  blank()
  ...
  if (is.null(fs) && !is.null(values$data)) {
    ...
    if (n_miss == 0) {
      ...
      add("Of ", n_cell, " participant-by-time cells, ", n_miss, " (",
          dance_apa_num(100 * n_miss / n_cell, 1), "%) were missing, affecting ",
  ...
  if (!is.null(fs)) {
    ...
    add("Of ", n_cell, " cells, ", cnt("observed"), " (",
        dance_apa_num(100 * cnt("observed") / n_cell, 1), "%) were measured, ",
        cnt("interpolated"), " (", dance_apa_num(100 * cnt("interpolated") / n_cell, 1),
        "%) interpolated between observations and ", cnt("extrapolated"), " (",
        dance_apa_num(100 * cnt("extrapolated") / n_cell, 1),
  ...
  if (!is.null(values$group_variables) && length(values$group_variables)) {
    ...
    L <- c(L, dance_md_table(rows, c("l", "r", "l")))
  }
  ...
  if (!is.null(wr)) {
      ...
      landmark     = paste0("landmark registration, a monotone piecewise-linear map through matched landmarks",
                            if (!is.null(wr$landmarks_used) && length(wr$landmarks_used))
                              sprintf(" placed by hand at %s",
                                      paste(dance_apa_num(wr$landmarks_used, 3), collapse = ", "))
    ...
    if (identical(wr$method, "linear_shift") && ng > 1)
      add(" The shift was estimated on the ", ng, "-point analysis grid, so ",
          ...
          dance_apa_num(1 / (ng - 1), 4, bounded = TRUE),
  ...
  if (!is.null(pw)) {
    ...
    add("**Post-hoc comparisons.** All ", m, " pairwise comparisons were tested ",
        ...
        "controlled by the ", dance_apa_method(pw$correction_method), " correction.")
  ...
  if (!is.null(pa)) {
    add("**Component scores.** Group differences on the functional principal ",
        ...
        dance_apa_method(pa$across_pc_correction), " method, and pairwise comparisons within a ",
        "component were computed only where the across-component adjusted ",
        "omnibus *p* passed a gate of ",
        dance_apa_num(pa$posthoc_gate %|% .05, 2, bounded = TRUE),
        ", using the ", dance_apa_method(pa$posthoc_method), " correction.")
    blank()
  }
  ...
  if (!is.null(sm) && is.finite(sm$mean_r_squared %|% NA)) {
    ...
    add("The smoother reproduced the observed series closely, ",
        "*R*<sup>2</sup> = ", dance_apa_num(sm$mean_r_squared, 3, bounded = TRUE),
        if (is.finite(sm$sd_r_squared %|% NA))
          paste0(" (*SD* = ", dance_apa_num(sm$sd_r_squared, 3, bounded = TRUE), ")") else "",
        ", *RMSE* = ", dance_apa_num(sm$mean_rmse, 3),
        if (is.finite(sm$sd_rmse %|% NA))
          paste0(" (*SD* = ", dance_apa_num(sm$sd_rmse, 3), ")") else "", ".",
        if (is.finite(sm$mean_df %|% NA))
          paste0(" The fitted curves used ", dance_apa_num(sm$mean_df, 1),
  ...
  if (!is.null(fa)) {
    ...
    add("Comparing ", fa$n_groups, " ",
        ...
        dance_apa_num(100 * ns / nt, 1), "% of the interval).")
    ...
    if (length(idx) == 1 && is.finite(fa$F_stat[idx])) {
      ...
      add(" The largest difference occurred",
          ...
          dance_apa_F(fa$F_stat[idx], fa$df_between, fa$df_within,
                    fa$p_values_adjusted[idx]), " (FDR-adjusted).")
    }
    add(" ", if (identical(fa$eta_squared_type, "partial"))
              "Partial eta-squared" else "Eta-squared",
        " averaged ", dance_apa_num(mean(fa$eta_squared, na.rm = TRUE), 3, bounded = TRUE),
        " across the interval (range ", dance_apa_range(fa$eta_squared, 3, TRUE), ").")
    if (is.finite(fa$L2_stat %|% NA))
      add(" The global *L*<sup>2</sup> statistic was ",
          dance_apa_num(fa$L2_stat, 3),
          if (is.finite(fa$p_value_L2 %|% NA))
            paste0(", ", dance_apa_p(fa$p_value_L2), ".")
  ...
  if (!is.null(pw)) {
    ...
    rows <- do.call(rbind, lapply(pw$pair_names, function(nm) {
      ...
      data.frame(Comparison = nm, `n₁` = r$n1, `n₂` = r$n2,
                 `L²` = dance_apa_num(r$L2_stat, 3),
                 `p` = dance_apa_pval(r$p_value_L2),
                 `p adj` = dance_apa_pval(r$p_value_L2_adjusted),
                 check.names = FALSE, stringsAsFactors = FALSE)
    }))
    ...
    add(nsig, " of ", length(pw$pair_names), " comparisons remained significant ",
        "after the ", dance_apa_method(pw$correction_method), " correction at alpha = ",
        dance_apa_num(pw$alpha, 2, bounded = TRUE), ".")
    ...
    if (n_at > 0)
      add(" ", n_at, " comparison", if (n_at == 1) "" else "s",
          ...
          dance_apa_pval(fl, 4), "); ", if (n_at == 1) "its true value is" else "their true values are",
  ...
  if (!is.null(values$pca_results)) {
    ...
    if (nk >= 1) {
      ...
      add("The first ", nk, " functional principal component",
          ...
          dance_apa_num(100 * sum(vp), 1), "% of the variance",
          if (!is.null(wr))
      ...
      rows <- data.frame(Component = paste0("PC", seq_len(nk)),
                         `Variance (%)` = dance_apa_num(100 * vp, 1),
                         `Cumulative (%)` = dance_apa_num(100 * cumsum(vp), 1),
                         check.names = FALSE, stringsAsFactors = FALSE)
      blank(); L <- c(L, dance_md_table(rows)); blank()
  ...
  if (!is.null(pa) && !is.null(pa$k)) {
    ...
    rows <- do.call(rbind, lapply(seq_len(pa$k), function(j) {
      ...
      data.frame(Component = paste0("PC", j),
                 `F` = if (is.null(o)) "--" else dance_apa_num(o$F, 2)[1],
                 # dance_pc_omnibus() names these df1/df2, not df_between/df_within;
                 ...
                 `p` = dance_apa_pval(pa$p_omnibus[j]),
                 `p adj` = dance_apa_pval(pa$p_omnibus_adj[j]),
                 `ω²` = if (is.null(o)) "--" else dance_apa_num(o$omega2, 3, bounded = TRUE)[1],
                 check.names = FALSE, stringsAsFactors = FALSE)
    }))
    L <- c(L, dance_md_table(rows)); blank()
    add("*p* values are adjusted across components by the ",
        dance_apa_method(pa$across_pc_correction), " method.")
    blank()
  ...
  if (!is.null(hm) && !is.null(hm$individual_fits)) {
    ...
    if (sum(ok) > 0) {
      add(" MESOR ", dance_apa_M(g1("mesor")), "; amplitude ",
          dance_apa_M(g1("amplitudes")), "; acrophase ",
          dance_apa_M(g1("acrophases_time")), " h.")
      ...
      add(" A detectable rhythm (zero-amplitude *F* test, alpha = .05) was ",
          ...
          dance_apa_num(100 * nsig / sum(ok), 1), "%). Model fit was ",
          "*R*<sup>2</sup> ", dance_apa_M(g1("r_squared"), 3, bounded = TRUE), ".")
    }

    # ---- the joint confidence regions the app already computes --------------
    # AUDIT (P12.2): dance_bingham_ci() builds the error ellipse for every
    ...
    if (!is.null(bs) && length(bs) >= 1 && !is.null(bs[[1]])) {
      ...
      add(" Joint 95% confidence regions (first harmonic) had a median ",
          "half-width of ", dance_apa_num(b1$median_amp_halfwidth, 3),
          " for the amplitude",
          if (is.finite(b1$median_acro_halfwidth_h %|% NA_real_))
            paste0(" and ", dance_apa_num(b1$median_acro_halfwidth_h, 2),
    ...
    if (!is.null(ms) && nrow(ms) > 0) {
      ...
      mrows <- data.frame(
        ...
        AICc     = dance_apa_num(ms$AICc, 2),
        `ΔAICc`  = dance_apa_num(ms$dAICc, 2),
        `Akaike weight` = dance_apa_num(ms$weight, 3, bounded = TRUE),
        check.names = FALSE, stringsAsFactors = FALSE)
      L <- c(L, dance_md_table(mrows)); blank()
      ...
      if (!is.null(sel) && !identical(sel, best)) {
        ...
        add("The specification reported above (", sel, ") is not the ",
            ...
            "reported model is ", dance_apa_num(d, 2), " AICc units behind it ",
            "with an Akaike weight of ",
            dance_apa_num(ms$weight[ms$model == sel], 3, bounded = TRUE),
    ...
    if (!is.null(hm$group_var_name) && length(gf) >= 1) {
      ...
      grows <- do.call(rbind, lapply(names(gf), function(nm) {
        ...
        data.frame(
          Group      = nm,
          `n`        = g$n %|% NA,
          `MESOR`    = dance_apa_num(g$rhythm_adjusted_mean %|% g$intercept %|% NA_real_, 2),
          `Amplitude`= paste0(dance_apa_num(g$mean_amplitude %|% NA_real_, 2), " (",
                              dance_apa_num(g$sd_amplitude %|% NA_real_, 2), ")"),
          `Acrophase (h)` = dance_apa_num(g$mean_acrophase_time %|% NA_real_, 2),
          check.names = FALSE, stringsAsFactors = FALSE)
      }))
      L <- c(L, dance_md_table(grows)); blank()
  ...
  if (!is.null(rg) && !is.null(rg$beta.hat)) {
    ...
    if (identical(rg$inference, "analytic-t-fdr")) {
      rows <- do.call(rbind, lapply(seq_len(nrow(bh)), function(i) {
        ...
        data.frame(Coefficient = rownames(bh)[i],
                   `β range` = dance_apa_range(bh[i, ], 3),
                   `Points p adj < .05` = sprintf("%d of %d",
                                                  sum(pp < .05, na.rm = TRUE), length(pp)),
                   `Min p adj` = dance_apa_pval(min(pp, na.rm = TRUE)),
                   check.names = FALSE, stringsAsFactors = FALSE)
      }))
      L <- c(L, dance_md_table(rows)); blank()
    ...
    if (!is.null(rg$r2_t))
      add(" Pointwise *R*<sup>2</sup> averaged ",
          dance_apa_num(mean(rg$r2_t, na.rm = TRUE), 3, bounded = TRUE),
          " (range ", dance_apa_range(rg$r2_t, 3, TRUE), ").")
    blank()
  ...
  if (!is.null(wr) && !is.null(wr$fit_statistics)) {
    ...
    add("Registration reduced between-curve dispersion by ",
        ...
        else paste0(dance_apa_num(100 * g, 1), "%"),
        " (*V*<sub>pre</sub> = ", dance_apa_num(st$total_dispersion_pre, 3),
        ", *V*<sub>post</sub> = ", dance_apa_num(st$total_dispersion_post, 3),
        "). The time axis moved by ",
        dance_apa_num(st$mean_phase_displacement, 4),
  ...
  rows <- do.call(rbind, c(
    ...
    lapply(pkgs, function(p) {
      ...
      if (is.na(v)) NULL else data.frame(Package = p, Version = v,
                                         stringsAsFactors = FALSE) })))
  L <- c(L, dance_md_table(rows, c("l", "r"))); blank()
  ...
  add("---")
  blank()
  add("*Prepared with DANCE. Check every number against the app before ",
...
# ==============================================================================
# dance_apa_html(md_lines, title) -- a standalone HTML rendering
# ==============================================================================
...
dance_apa_html <- function(md_lines, title = "Analysis report") {
  ...
  # A literal asterisk is written "\\*" in the Markdown (a column name or a
  # DANCE). Leaving it in place while the italic pass runs makes the emphasis

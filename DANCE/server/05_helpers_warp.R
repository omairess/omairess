...
# reactive observer.
#
# P11.2 -- THE LIVE DEFECT. dance_landmark_warp() was defined at depth 11, inside
...
# ==========================================================================
# dance_landmark_warp(ref, own, time_points)
#
...
# ==========================================================================
dance_landmark_warp <- function(ref, own, time_points) {
...
parametric_alignment <- function(fd_obj, family = "power", 
  ...
  tryCatch({
    ...
    # the same pattern dance_auto_lambda() already uses for the GCV search.
    dance_min_1d <- function(f, lo, hi, n_grid = 41) {
    ...
    for(i in 1:n_curves) {
      ...
      a_hat <- dance_min_1d(objective, param_range[1], param_range[2])
  ...
  tryCatch({
    ...
    if(!is.null(landmark_points) && nrow(landmark_points) > 0) {
      ...
      for(i in 1:n_curves) {
        ...
        # and is counted.
        h <- dance_landmark_warp(landmark_times, curve_landmarks, time_points)
        if (is.null(h)) {
      ...
      for(i in 1:n_curves) {
        ...
        h <- dance_landmark_warp(ref_lm, own, time_points)
        if (is.null(h)) {
          n_rejected <- n_rejected + 1L
          rejected_ids <- c(rejected_ids, i)
          h <- time_points

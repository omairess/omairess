...
# ==========================================================================
  ...
  output$rm_factor_levels_ui <- renderUI({
    ...
    rm_factor_data <- dance_rm_column(values, input$rm_factor_var)
  ...
  perform_rm_fanova <- function(fd_obj, subject_id, rm_factor, n_permutations = 200,
    ...
    tryCatch({
      ...
      L2_stat <- dance_l2_norm(
        ifelse(is.finite(SS_visit_t) & is.finite(n_complete_t) & n_complete_t > 0,
               SS_visit_t / n_complete_t, 0),
        time_points)
      L2_stat_classical <- dance_l2_norm(SSB_classical / length(subject_id), time_points)
      ...
      if (isTRUE(run_global_test)) {
        rm_global <- tryCatch(
          dance_rmfanova_global(curves, subject_id, rm_factor,
  ...
  observeEvent(input$run_fanova, {
    ...
    tryCatch({
      ...
      if(is.null(fd_to_use)) {
        ...
        # MERGED APP: when it has not, dance_ensure_fd_obj() builds an
      ...
      if(input$fanova_design == "between") {
        ...
        # MERGED APP: aligned to the curves (see DANCE/server/52_posthoc_source.R).
        subject_id_data <- dance_rm_column(values, input$rm_subject_id_var)
        rm_factor_data  <- dance_rm_column(values, input$rm_factor_var)
    ...
    if (!is.null(res$rm_global)) {
      ...
      if (!isTRUE(g$ok)) {
        ...
        cat(sprintf("  %s rejected true nulls at 17.5%% and 14.2%% (nominal 5%%)\n",
                    paste(DANCE_RMFANOVA_INFLATED, collapse = " and ")))
        cat(sprintf("  %s had no power at all in that setting.\n",
                    paste(DANCE_RMFANOVA_NOPOWER, collapse = " and ")))
  ...
  observeEvent(input$run_pairwise, {
    ...
    tryCatch({
      ...
      # (DANCE/server/52_posthoc_source.R). Before this, the between-subjects
      ...
      spec <- dance_posthoc_spec(input, values)
      if (!isTRUE(spec$ok)) {

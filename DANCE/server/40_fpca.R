...
# ==========================================================================
  ...
  output$group_preview_plot <- renderPlot({
    if(!is.null(values$data) && !is.null(values$group_labels)) {
      ...
      colors <- unname(dance_group_colors(groups)[groups])
  ...
  observeEvent(input$run_analysis, {
    ...
    tryCatch({
      ...
      if(is.null(values$fd_obj)) {
        # MERGED APP: one rule for this, in DANCE/server/04_helpers_fd.R. No
        ...
        if(!dance_ensure_fd_obj(values)) return()
      ...
      if(input$pca_type == "fpca") {
        ...
        if(warping_method == "linear_shift") {
          ...
          # P11.2: the kernel is pure, so the landmarks it should use are passed
          # in rather than read from `values` inside it, and it RETURNS its
          # rejection message rather than raising a notification itself.
          warp_fd <- landmark_alignment_simple(values$fd_obj, landmarks, time_grid,
                                               landmark_points = values$landmark_points)
          if (!is.null(warp_fd$rejection_warning))
            showNotification(warp_fd$rejection_warning, type = "warning", duration = 15)
        }
  ...
  observeEvent(values$pca_results, {
    ...
    n_avail <- min(ncol(pr$scores), length(pr$values), dance_n_harmonics(pr))
    ...
    if(is.null(values$pca_results)) {
      ...
      tryCatch({
        ...
        if(!is.null(pca_res$varprop)) {
          ...
          nk <- min(dance_n_harmonics(pca_res), length(pca_res$varprop))
  ...
  output$loadings_plot <- renderPlotly({
    ...
    tryCatch({
      ...
      colors <- dance_component_colors(max(1, min(ncol(pca_res$scores), dance_n_harmonics(pca_res))))
      ...
      n_comp <- min(ncol(pca_res$scores), dance_n_harmonics(pca_res))
      
      for(i in 1:n_comp) {
        if(i <= dance_n_harmonics(pca_res)) {
    ...
    tryCatch({
      ...
      p <- p %>%
        add_trace(x = 1:n_show,
                  ...
                  name = 'Cumulative',
                  yaxis = 'y2',
                  line = list(color = DANCE_EMPHASIS),
                  marker = list(color = DANCE_EMPHASIS))
  ...
  output$scores_plot <- renderPlotly({
    ...
    tryCatch({
      ...
      n_avail <- min(ncol(pca_res$scores), length(pca_res$values), dance_n_harmonics(pca_res))
      ...
      colors <- dance_group_ramp(max(n_show, 1))
  ...
  output$warping_scores <- renderDT({
    ...
    tryCatch({
      ...
      warp_amplitude <- dance_warp_amplitude(warp_results)
  ...
  output$warping_model_criteria <- renderText({
    ...
    tryCatch({
      ...
      paste0(
        ...
        # AUDIT (P11.4). A reviewer pointed out, correctly, that the shift
        # estimate has a floor no amount of correcting P10.1 can move: the FFT
        # is evaluated on the analysis grid, so the smallest non-zero lag it
        # can return is one grid step. On 24 hourly columns that is 1/23 of a
        # cycle, about 63 minutes; on 17 columns it is 1.5 hours. That is not
        # an indexing error -- it is the resolution of the grid the estimator
        # is given -- but it is invisible in a table of four-decimal shifts,
        # and reading a 0.04-cycle difference as a finding when the estimator
        # cannot resolve better than 0.043 is exactly the overstatement this
        # audit exists to prevent. So it is reported next to the number.
        #
        # The grid is NOT silently densified. Because the app holds an fd
        # object, the curves could be evaluated on 256 points and the shifts
        # would come back on a finer lattice -- but that is finer INTERPOLATION
        # of the same fitted curves, not more measurement, and it would change
        # every number a user has already reported. If that trade is worth
        # making it should be a visible choice, not a silent one.
        {
          n_grid <- length(values$warping_results$time_points)
          if (is.null(n_grid) || n_grid < 2) "" else paste0(
            "\n",
            "Resolution of the phase estimate\n",
            "================================\n",
            "  The lag is estimated on the analysis grid, so the smallest\n",
            "  non-zero shift it can return is one grid step:\n",
            sprintf("    %d points -> %.4f of the domain\n", n_grid, 1 / (n_grid - 1)),
            sprintf("    on a 24-hour cycle, %.0f minutes\n", 24 * 60 / (n_grid - 1)),
            "  Differences smaller than that are below the resolution of the\n",
            "  estimate and should not be interpreted, however many decimals\n",
            "  the table prints. Evaluating the fitted curves on a denser grid\n",
            "  would refine the lattice but adds no measurement.\n")
        },
        "\n",
      ...
      tryCatch({
        ...
        if(!is.null(values$warping_results) && !is.null(values$warping_results$warp_functions)) {
          ...
          matplot(time_points, warp_functions[,1:min(30, ncol(warp_functions))], 
                  type = "l", col = dance_group_rgba(DANCE_SERIES1, 0.30), lty = 1,
                  xlab = "Original Time", ylab = "Warped Time",
                  main = "Time Warping Functions")
          lines(c(0,1), c(0,1), col = DANCE_NEUTRAL, lwd = 2, lty = 2)
          legend("topleft", legend = c("Individual", "Identity"), 
                 col = c(DANCE_SERIES1, DANCE_NEUTRAL), lty = c(1, 2), lwd = c(1, 2))
        } else {
  ...
  calculate_warping_fit_statistics <- function(original_curves, registered_curves,
    ...
    tryCatch({
      ...
      for(i in 1:n_subjects) {
        ...
        #
        # This is not only cosmetic. dance_warp_params() feeds warp_amplitude into
        ...
        # exactly what dance_warp_amplitude() returns for a shift, computed above.
        if (identical(geom, "shift")) {
  ...
  # AUDIT (P11.1/P11.2): linear_shift_alignment(), parametric_alignment(),
  # landmark_alignment_simple() and dance_landmark_warp() moved to
  # server/05_helpers_warp.R. They are pure, so they can be shared by the app,
  # the tests and the exported script -- see the header of that file for what
  # went wrong while they lived here.
  ...
  output$warping_plot <- renderPlotly({
    ...
    tryCatch({
      ...
      p <- p %>% add_trace(
        ...
        line = list(color = DANCE_NEUTRAL, width = 2, dash = 'dash'),
    ...
    tryCatch({
      ...
      if(is.null(input$selected_subject) || is.null(input$landmark_target) ||
        ...
        p <- plot_ly(source = "landmark_source") %>%
          add_trace(x = time_points, y = mean_curve, 
                    ...
                    name = 'Mean curve',
                    line = list(color = DANCE_EMPHASIS, width = 2))
      ...
      if(!is.null(values$landmark_points) && nrow(values$landmark_points) > 0) {
        p <- p %>% add_trace(x = values$landmark_points$x,
                             ...
                             marker = list(color = DANCE_SERIES2, size = 10),
                             name = 'Landmarks')
      }
